...
describe("AdminApiClient", () => {
  ...
  it("builds request chain urls with encoded selector params", async () => {
    const fetchImpl = vi.fn().mockResolvedValue(
      new Response(JSON.stringify({ api_version: 1, requests: [] }), {
        headers: { "content-type": "application/json" },
        status: 200,
      }),
    );
    ...
    await client.getRequestLedgerChain(undefined, {
      limit: 10,
    ...
    expect(fetchImpl).toHaveBeenCalledWith(
      new URL(
        "http://127.0.0.1:4211/__codex_helper/api/v1/request-ledger/chain?limit=10&trace_id=trace%2Fwith+space&request_id=42&session=session+a",
      ),
      expect.objectContaining({ headers: { accept: "application/json" } }),
    );
  });
