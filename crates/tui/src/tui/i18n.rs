...
define_messages! {
    ...
    FOOTER_STATS => { zh: "1-8 页面  q 退出  L 语言  Tab 焦点(station/provider)  ↑/↓ 选择  d 窗口(today/7d/30d/loaded)  e 仅看错误(recent)  y 复制+导出报告  ? 帮助", en: "1-8 pages  q quit  L language  Tab focus(station/provider)  ↑/↓ select  d window(today/7d/30d/loaded)  e errors_only(recent)  y copy+export report  ? help" },
    FOOTER_SETTINGS_CODEX => { zh: "1-8 页面  q 退出  L 语言  p 配置默认profile  P 运行时默认profile  R 重载配置  O 覆盖导入(~/.codex，二次确认)  ? 帮助", en: "1-8 pages  q quit  L language  p configured-default-profile  P runtime-default-profile  R reload  O overwrite(~/.codex, confirm)  ? help" },
    FOOTER_SETTINGS_OTHER => { zh: "1-8 页面  q 退出  L 语言  p 配置默认profile  P 运行时默认profile  R 重载配置  ? 帮助", en: "1-8 pages  q quit  L language  p configured-default-profile  P runtime-default-profile  R reload  ? help" },
    FOOTER_HISTORY => { zh: "1-8 页面  q 退出  L 语言  ↑/↓ 选择  r 刷新  t/Enter 对话记录  s 打开到 Sessions  f 打开到 Requests  ? 帮助", en: "1-8 pages  q quit  L language  ↑/↓ select  r refresh  t/Enter transcript  s open Sessions  f open Requests  ? help" },
    FOOTER_RECENT => { zh: "1-8 页面  q 退出  L 语言  ↑/↓ 选择  [] 切换时间  r 刷新  Enter 复制选中  y 复制全部(可见)  t transcript  s/f/h 跳转  ? 帮助", en: "1-8 pages  q quit  L language  ↑/↓ select  [] window  r refresh  Enter copy selected  y copy all(visible)  t transcript  s/f/h navigate  ? help" },
