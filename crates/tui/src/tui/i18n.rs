...
define_messages! {
    ...
    FOOTER_STATS => { zh: "1-8 页面  q 退出  L 语言  Tab 焦点(station/provider)  ↑/↓ 选择  d 窗口(today/7d/30d/loaded)  g 刷新余额  e 仅看错误(recent)  y 复制+导出报告  ? 帮助", en: "1-8 pages  q quit  L language  Tab focus(station/provider)  ↑/↓ select  d window(today/7d/30d/loaded)  g refresh balances  e errors_only(recent)  y copy+export report  ? help" },
