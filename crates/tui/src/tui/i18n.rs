...
define_messages! {
    ...
    FOOTER_SETTINGS_CODEX => { zh: "1-8 页面  q 退出  L 语言  p/P profile  R 重载  O 导入  B/I/F/V/D patch  C 诊断  ? 帮助", en: "1-8 pages  q quit  L language  p/P profile  R reload  O import  B/I/F/V/D patch  C diagnose  ? help" },
    ...
    FOOTER_RECENT => { zh: "1-8 页面  q 退出  L 语言  ↑/↓ 选择  [] 时间  Enter/y 复制  t 记录  s/f/h 跳转  ? 帮助", en: "1-8 pages  q quit  L language  ↑/↓ select  [] window  Enter/y copy  t transcript  s/f/h navigate  ? help" },
    ...
    COMMON_KEYS_CODEX => { zh: "  1-8 切页  ? 帮助  q 退出  L 语言  (Stations: i 详情  Usage: y 导出/复制  Settings: R 重载  O 覆盖导入  C 诊断  B/I/F/V/D patch)", en: "  1-8 pages  ? help  q quit  L language  (Stations: i details  Usage: y export/copy  Settings: R reload  O overwrite  C diagnose  B/I/F/V/D patch)" },
    COMMON_KEYS_OTHER => { zh: "  1-8 切页  ? 帮助  q 退出  L 语言  (Stations: i 详情  Usage: y 导出/复制)", en: "  1-8 pages  ? help  q quit  L language  (Stations: i details  Usage: y export/copy)" },
