...
use crate::tui::input::codex_switch_intent_for_key;
...
async fn removed_runtime_mutation_keys_are_inert() {
    ...
    let cases = [
        ...
        (Page::Stations, vec!['r', 'g', 'h', 'H', 'c', 'C', 'o', 'O']),
        (
            Page::Sessions,
...
#[tokio::test]
async fn station_details_remain_a_read_only_overlay() {
    let mut ui = UiState {
        page: Page::Stations,
        ..UiState::default()
    };
    ...
    assert_eq!(ui.overlay, Overlay::StationInfo);
    assert!(press(&mut ui, &snapshot, KeyCode::Esc).await);
    assert_eq!(ui.overlay, Overlay::None);
}
