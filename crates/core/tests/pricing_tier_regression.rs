...
use codex_helper_core::pricing::{
    ...
    PriceMultiplier, UsdAmount, estimate_usage_cost_with_convention,
};
use codex_helper_core::usage::{CacheAccountingConvention, CacheTokenInclusion, UsageMetrics};
...
fn pricing_context_tier_boundary_uses_whole_request_and_strict_greater_than() {
    ...
    let base = estimate_usage_cost_with_convention(
        &base_usage,
        &price,
    ...
    let tiered = estimate_usage_cost_with_convention(
        &tiered_usage,
        &price,
        CostAdjustments::default(),
        CacheAccountingConvention::SEPARATE,
...
fn pricing_combines_fractional_multipliers_before_one_final_rounding() {
    ...
    let multi_component = estimate_usage_cost_with_convention(
        &UsageMetrics {
            input_tokens: 1_000_000,
    ...
    let fractional_boundary = estimate_usage_cost_with_convention(
        &UsageMetrics {
            input_tokens: 1_000_000,
            ..UsageMetrics::default()
        },
