...
fn recovered_candidate_error(
    path: &Path,
    write_error: &str,
    detail: impl std::fmt::Display,
) -> io::Error {
    io::Error::other(format!(
        "{write_error}; failed to recover uncertain write to {}: {detail}",
        path.display()
    ))
}

fn validate_recovered_candidate<V>(
    path: &Path,
    candidate: &[u8],
    recovered: &[u8],
    write_error: &str,
    ...
    V: FnOnce(&[u8]) -> io::Result<()>,
{
    if recovered != candidate {
        return Err(recovered_candidate_error(
            path,
            write_error,
            "destination bytes do not match the candidate",
        ));
    }
    validate(recovered).map_err(|err| recovered_candidate_error(path, write_error, err))
}

pub(crate) fn recover_uncertain_candidate<V>(
    path: &Path,
    candidate: &[u8],
...
where
    V: FnOnce(&[u8]) -> io::Result<()>,
{
    if !matches!(&error, AtomicWriteError::CommitStateUnknown { .. }) {
        return Err(io::Error::other(error));
    }
    let write_error = error.to_string();
    let recovered =
        fs::read(path).map_err(|err| recovered_candidate_error(path, &write_error, err))?;
    validate_recovered_candidate(path, candidate, &recovered, &write_error, validate)
}

pub(crate) async fn recover_uncertain_candidate_async<V>(
    path: &Path,
    candidate: &[u8],
    error: AtomicWriteError,
    validate: V,
    ...
    V: FnOnce(&[u8]) -> io::Result<()>,
{
    ...
    let write_error = error.to_string();
    let recovered = tokio::fs::read(path)
        .await
        .map_err(|err| recovered_candidate_error(path, &write_error, err))?;
    validate_recovered_candidate(path, candidate, &recovered, &write_error, validate)
}
