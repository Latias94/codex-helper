...
struct CostBreakdownWire {
    ...
    #[serde(default)]
    pricing_generation: Option<String>,
    #[serde(default)]
    effective_pricing_revision: Option<String>,
    #[serde(default)]
    selected_tier: Option<SelectedPriceTier>,
}
...
impl<'de> Deserialize<'de> for CostBreakdown {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        Ok(Self {
            ...
            pricing_provider: wire.pricing_provider,
            pricing_generation: wire.pricing_generation,
            effective_pricing_revision: wire.effective_pricing_revision,
            selected_tier: wire.selected_tier,
...
impl PartialEq for CostBreakdown {
    fn eq(&self, other: &Self) -> bool {
        self.input_cost_usd == other.input_cost_usd
            ...
            && self.pricing_provider == other.pricing_provider
            && self.pricing_generation == other.pricing_generation
            && self.effective_pricing_revision == other.effective_pricing_revision
            && self.selected_tier == other.selected_tier
    }
}
...
impl CapturedModelPriceCatalog {
    ...
    #[cfg(test)]
    pub(crate) fn with_test_revision(mut self, revision: impl Into<String>) -> Self {
    ...
    pub fn estimate_usage_cost_for_service_with_convention(
        &self,
        service: &str,
        model: &str,
        usage: &UsageMetrics,
        ...
        let cost = self
            .catalog
            .estimate_usage_cost_for_service_with_convention(
                service,
                model,
                usage,
                adjustments,
...
impl EffectivePricingCatalogSnapshot {
    ...
    pub fn estimate_request_cost(
        ...
        let cost = match (model, usage) {
            (Some(model), Some(usage)) => self.catalog.estimate_usage_cost_with_convention(
                model,
                usage,
                adjustments,
                convention,
...
pub fn basellm_model_price_catalog_snapshot(
    snapshot: &BasellmCatalogLkg,
) -> ModelPriceCatalogSnapshot {
    ModelPriceCatalog::with_prices(project_basellm_model_prices(snapshot))
        .snapshot("basellm-import")
}
...
pub fn try_capture_operator_model_price_catalog() -> Result<CapturedModelPriceCatalog, String> {
    let snapshot = refresh_effective_pricing_catalog();
    if snapshot.manual_status == ManualPricingLayerStatus::Invalid {
        return Err(snapshot
            .manual_error
            .clone()
            .unwrap_or_else(|| "invalid manual pricing layer".to_string()));
    }
    Ok(capture_effective_pricing_snapshot(&snapshot))
}
...
pub fn capture_operator_model_price_catalog() -> CapturedModelPriceCatalog {
    match try_capture_operator_model_price_catalog() {
        ...
        Err(err) => {
            ...
            capture_effective_pricing_snapshot(&effective_pricing_catalog_snapshot())
        }
    }
}

fn capture_effective_pricing_snapshot(
    snapshot: &EffectivePricingCatalogSnapshot,
...
pub fn estimate_request_cost_from_operator_catalog(
    ...
    estimate_request_cost_from_operator_catalog_with_convention(
        model,
        usage,
        adjustments,
        CacheAccountingConvention::UNKNOWN,
    )
}
...
pub fn estimate_request_cost_from_operator_catalog_with_convention(
    ...
    snapshot.estimate_request_cost(model, usage, adjustments, convention)
}
...
pub fn estimate_request_cost_from_operator_catalog_for_service(
    ...
    estimate_request_cost_from_operator_catalog_for_service_with_convention(
        model,
        usage,
        adjustments,
        service,
...
pub fn estimate_request_cost_from_operator_catalog_for_service_with_convention(
    model: Option<&str>,
    usage: Option<&UsageMetrics>,
    adjustments: CostAdjustments,
    service: &str,
    ...
    let snapshot = effective_pricing_catalog_snapshot();
    snapshot.estimate_request_cost_for_service(service, model, usage, adjustments, convention)
}
...
pub fn estimate_usage_cost_from_captured_provider_price(
    ...
    price.source_generation = Some(quote.pricing_revision().as_str().to_string());
...
mod tests {
    ...
    #[test]
    fn captured_operator_catalog_stamps_its_frozen_revision_on_cost() {
        ...
        .expect("price");
        let captured = CapturedModelPriceCatalog {
            catalog: ModelPriceCatalog::with_prices([price]),
            source: "captured-test".to_string(),
            revision: "sha256:captured-test".to_string(),
        };
        ...
        let cost = captured.estimate_usage_cost_with_convention(
            "captured-model",
            &usage,
            CostAdjustments::default(),
        ...
        assert_eq!(
            cost.effective_pricing_revision.as_deref(),
            Some("sha256:captured-test")
        );
    }
    ...
    fn provider_epoch_standard_price_bills_exclusive_gpt_5_6_cache_buckets_once() {
        ...
        assert_eq!(
            cost.pricing_generation.as_deref(),
    ...
    #[test]
    fn missing_required_cache_price_with_unknown_accounting_makes_cost_unknown() {
        let price =
