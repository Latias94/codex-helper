...
impl AttributionIndex {
    ...
    pub(super) fn record(&mut self, request: &FinishedRequest) -> AttributionInsert {
        let request_key = request_identity_key(request);
        if self.seen_traces.contains_key(&request_key) {
        ...
        self.seen_traces
            .insert(request_key.clone(), request.ended_at_ms);
        self.trace_order
            .push_back((request.ended_at_ms, request_key));
...
pub(super) fn request_identity_key(request: &FinishedRequest) -> String {
...
mod tests {
    ...
    #[test]
    fn duplicate_request_identity_keeps_captured_pool_revision() {
