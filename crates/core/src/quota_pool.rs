...
/// In-memory quota state machine.  Callers should serialize the exported checkpoint through
/// `QuotaSampleStore`; this type intentionally has one mutating ingestion method.
#[derive(Debug, Clone)]
...
mod tests {
    ...
    fn snapshot(at: u64, used: &str) -> ProviderBalanceSnapshot {
        let mut snapshot = ProviderBalanceSnapshot::new("relay", "station", 0, "test", at, None);
    ...
    fn unlimited_quota_is_retained_without_fabricated_amounts() {
        ...
        let mut unlimited = ProviderBalanceSnapshot::new("relay", "station", 0, "test", 100, None);
    ...
    #[test]
    fn quantity_decimal_round_trips_without_float() {
        let quantity = QuotaQuantity::from_decimal("12.3400", QuotaUnit::Usd).expect("decimal");
        assert_eq!(quantity.value, 1234);
