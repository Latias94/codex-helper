...
- Codex switch surface 收敛为 `switch on [--port | --base-url]`、`switch off` 和 `switch status`，只管理 helper 自有的 provider selector/stanza。`switch remote-control`、preset/mode/compaction/WebSocket 等客户端假设参数、`[codex.client_patch]`、`[features].remote_connections` 和直接读写 Codex App SQLite 的 hack 均已删除；下方旧版本中关于这些接口的条目仅保留为历史记录，已被本项取代。0.20.3 及更早版本如仍有 `~/.codex/codex-helper-switch-state.json`，必须先用旧 binary 执行 `switch off` 再升级；该文件可能包含原始 auth 内容，不得直接删除或分享。
