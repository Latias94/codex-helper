...
- Version 5 现在是一次性迁移输入：启动先保留精确 `.bak`，再生成唯一运行时契约 version 6。`[codex.client_patch]` 会保留并把旧 `mode`、underscore/bridge alias、compaction 与 hosted-image alias 规范化为 canonical 值；独立 `[codex.compaction]` / `[claude.compaction]`、`[ui.usage_forecast]` 和退休 retry/profile/relay-target 字段仍会精确清理。迁移保留已有凭据语义和值；只懂 v5 的旧 binary 会拒绝 v6，降级必须恢复迁移前备份。
...
- Version 5 is now a one-time migration input. Startup preserves an exact `.bak` and produces the sole version 6 runtime contract. `[codex.client_patch]` is retained while legacy `mode`, underscore/bridge aliases, compaction aliases, and hosted-image aliases normalize to canonical values; standalone `[codex.compaction]` / `[claude.compaction]`, usage-forecast, and retired retry/profile/relay-target fields are still removed precisely. Existing credential semantics and values are preserved. A v5-only binary rejects v6, so downgrade requires restoring the pre-migration backup.
