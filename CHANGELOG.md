...
- Version 5 现在是一次性迁移输入：启动先保留精确 `.bak`，再生成唯一运行时契约 version 6，并精确清理 `[codex.client_patch]`、`[codex.compaction]`、`[claude.compaction]`、`[ui.usage_forecast]`、退休 retry/profile/relay-target 字段。迁移保留已有 inline/environment/client-file 凭据语义，不会生成引用、导入或删除凭据值；只懂 v5 的旧 binary 会拒绝 v6，降级必须恢复迁移前备份。
