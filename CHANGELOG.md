...
- Codex `/responses` 和 `/responses/compact` 请求现在会从已有请求证据补齐缺失的 `session_id`、`x-session-id` 和 `prompt_cache_key`；来源包括 header session、body `session_id`、`prompt_cache_key` 和 `metadata.session_id`。`previous_response_id` 只用于 stale-response 修复，不再作为 session completion 或 session identity 来源。helper 不会凭空生成 session id，也不会覆盖客户端已发送的 session 字段。
