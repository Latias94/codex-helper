...
- TUI `5 用量` 首屏现在展示远端共享 quota pool 的 used/remaining、15/60 分钟消耗速率、reset 前所需速率、pace、耗尽 ETA、source/scope/身份置信度和 freshness；按 `g` 可立即重新读取 operator read model，Projects 视图可查看本机 Git project 归因、unknown、external 和 signed gap。
...
- Route graph 新增容量加权 `round-robin`：按 `max_concurrent_requests - active_requests` 平滑分配冷 session，跳过饱和候选，并在首次成功后用 session affinity 粘住同一 key；一个 key 可以承载多个 session。
- 完整恢复 `[codex.client_patch]` 与 `switch on --preset ...`：五种 preset 可组合 `responses_websocket`、`compaction`、`translate_models` 和 `hosted_image_generation`，CLI/TUI/桌面端均显示生效状态。`chatgpt-bridge` 会验证并保留完整 ChatGPT 登录；imagegen preset 为 0.20.3 同期的旧 Codex 保留语义空 `{}` auth facade，当前 Codex 的 image/web extension gate 则是 `requires_openai_auth = false` 加非空 helper actor marker。原始 auth 与可能含敏感 header/注释的 inactive `codex_proxy` stanza 只存放在私有 helper backup，JSON journal 只保存指纹和随机文件名，CAS 可在关闭、重复应用与崩溃恢复时精确还原；helper actor marker 在本地消费且不上送。
- Routing TUI 改为适合长 provider 列表的密集 endpoint 表：宽屏使用候选列表/选中端点详情主从布局，窄屏退化为全宽表；同时展示路由顺序、偏好组、endpoint priority、并发容量、余额及样本年龄。进入页面会请求一次非强制刷新；停留期间样本缺失、过期或约六分钟未更新时，会以至少两分钟间隔再次委托 daemon 刷新。`g` 绕过 UI 自动刷新节流并强制全量刷新；远程 observer 保持只读，并支持分页与首尾导航。
- 配置契约升级到 `version = 6`，新增类型化 `native` / `secret_file` bearer 与 API-key 引用，以及不会在 argv 或输出中接收/显示凭据值的 `credential create/set/import/status/delete` 和 provider auth 绑定命令。
- Windows Credential Manager、macOS Keychain 和 Linux Secret Service 现在可供同一登录用户的本机 service 使用；service receipt、signed loopback refresh 和跨 CLI/service/TUI/desktop/Fleet 的脱敏 readiness 会区分 `ready`、`degraded` 与 `blocked`。
- `codex-helper-server --check [--json]` 可在不打开 SQLite、listener 或上游连接的情况下检查环境变量与绝对 secret-file 部署；Docker Compose 增加可选的只读 mounted-secret overlay 和真实 Engine smoke。
...
- Codex switch 现在管理完整 client patch，而不是仅管理 URL 与简化能力投影：不带覆盖时读取 version 6 配置，五项字段可由 `--preset`、`--compaction`、`--responses-websocket[=false]`、`--translate-models[=false]` 和 `--hosted-image-generation` 临时覆盖；旧 `--mode` 会给出明确迁移提示。`switch off` 只在当前有效 TOML 中恢复 helper-owned selector、stanza 与 feature keys，Codex 运行期产生的其它 provider、feature、project 和注释编辑会保留；受管字段冲突仍失败关闭。认证型 patch 关闭后保留私有恢复点并显示 `Off`，可修复 Codex 稍后重写的语义等价 facade；no-replace CAS 在崩溃和竞争写入时不会覆盖外部文件。相同 target 可安全重复应用不同 patch；更换 URL 仍需先关闭。`switch remote-control`、`[features].remote_connections` 管理和直接读写 Codex App SQLite 的 hack 保持删除。0.20.3 及更早版本留下的 v1/v2 `~/.codex/codex-helper-switch-state.json` 会在旧 patch 仍可验证时由 `switch on/off` 自动恢复；损坏、未知版本或新旧 journal 冲突会原样保留并失败关闭。
- 删除代码不会撤销旧 `switch remote-control enable` 已写入的 Codex `[features].remote_connections = true` 或 SQLite `remote_control` row。新版无法可靠判断这些状态是否仍由用户需要，因此不会自动清理；不要用 SQL hack 修改 Codex SQLite。
- 0.20.3 的 `state/session-route-affinities.json` 不会导入新的 `state.sqlite`，也不会被自动改写或删除。升级前应结束仍依赖旧多 endpoint affinity 的 state-bound 会话，升级后开启新会话；不要用相同 session key 发送普通请求来猜测性重绑，因为它可能选中不同 endpoint。
- Version 5 现在是一次性迁移输入：启动先保留精确 `.bak`，再生成唯一运行时契约 version 6。`[codex.client_patch]` 会保留并把旧 `mode`、underscore/bridge alias、compaction 与 hosted-image alias 规范化为 canonical 值；Codex 专用 `[codex.compaction]` 会保留并默认启用 v2→v1 fallback，显式 `false` 也会保留；只有不适用于 Claude 的 `[claude.compaction]`、`[ui.usage_forecast]` 和退休 retry/profile/relay-target 字段仍会精确清理。迁移保留已有凭据语义和值；只懂 v5 的旧 binary 会拒绝 v6，降级必须恢复迁移前备份。恢复旧备份不会删除 version 6 创建的 native-store entry，因此必须显式检查孤立逻辑名。
- Route graph 新增 `scheduling_preset = "continuity-first" | "balanced" | "throughput-first"`；新建或当前 version 6 配置缺省 `balanced`，会在本地并发上限饱和时最多等待 2 秒。从 pre-v6 配置迁移且该字段缺失时会显式写入 `throughput-first`，自动保留 0.20.3 的立即 failover 行为；已有显式值不会被覆盖。
- 持久 route affinity 改用版本化的 canonical SHA-256 route key；切换 `scheduling_preset`、调整 `max_concurrent_requests` / `limit_group`，或修改纯展示 alias/metadata 不再废掉已有 binding，而 route rule、endpoint identity 或配置内 `auth_token` / `api_key` 凭据变化仍会使旧 binding 保守失效。预发布 `state.sqlite` 中使用旧 key 格式的 affinity 不再匹配；客户端透传账号或外部 credentials fallback 发生变化时必须开启新会话。
- `usage_providers.json` 继续只读兼容安全的 v0.20.3 base/time/custom-variable 模板和非敏感 headers；URL 中的凭据会被拒绝，凭据模板只允许出现在 `Authorization` 或 `X-API-Key`。无效 provider 会单独隔离并显示诊断，其它 provider 仍可刷新，原文件不会被重写。
- 启动现在会把 v1-v5、无版本 TOML，以及缺少 canonical TOML 时的历史 `config.json` 自动迁移到 version 6；JSON-only `config init` 也会迁移并输出报告。写入前会验证完整结果并创建 `.bak`，损坏输入、未来版本、符号链接或并发源文件变化会 fail closed。历史 station-shaped JSON 只接受旧 loader 正式支持的 nullable 字段，provider-shaped JSON 中的 `null` 保守拒绝。只有 `[retry.upstream]` 不存在时，旧 `[retry]` flat 字段才会迁入该表；已存在的 nested 表会作为完整历史 override 保留。`config migrate --dry-run` 可脱敏预览，`config migrate --write --yes` 可显式执行；干净 v6 是 no-op。`config init` 不再执行旧式全量 Codex provider 导入，`--no-import` 仍已移除；独立的首次 `ch` onboarding 仅在 helper 尚无 Codex provider/route 时导入 Codex 当前选中的 provider，只保存 base URL 与凭据引用名称而不保存值。认证来源不明确或第三方 OpenAI-auth origin 不安全时失败关闭；helper route 可在启动失败后保留，但 Codex switch 只在 runtime 就绪后应用。
- 0.20.3 配置中无法安全猜测的信任与容量语义不会自动改写：远程 `relay_targets` 需显式可信 `admin_url`，启用的非 loopback fleet node 需 HTTPS 和有效 `admin_token_env`，同一显式 `limit_group` 的每个候选需声明相同 `max_concurrent_requests`。旧根级 `pricing_overrides.toml` `[models.*]` 仍按 OpenAI 读取，并在首次显式 pricing 写入时规范化为 provider-scoped v2；曾用于 Anthropic/Claude 的 row 需人工移动到对应 provider namespace。
- 远程第三方 Codex endpoint 在未配置 helper 凭据时默认不可路由，并在任何上游 I/O 前返回 503；确实不要求认证的 relay 必须显式设置 `allow_anonymous = true` 或使用 `provider add --allow-anonymous`。该 version 5 兼容语义会原样迁移到 version 6；官方 OpenAI 与 loopback 行为不变，失效的显式凭据引用也不会回退匿名。
- 本机安装 service、远端 host 和 Docker 是同一 server 产品的不同部署位置。每个接收请求的 server 独立拥有配置、runtime store、affinity、round-robin cursor、并发、health 和 quota；Fleet 保持只读观测，不协调跨 server 的凭据或容量。
...
- Route graph 编译现在拒绝同一显式 `limit_group` 中缺失或不同的 `max_concurrent_requests`，避免共享 semaphore 的容量由候选顺序偶然决定；无效 version 6 配置会失败关闭且不改写源文件。
...
- Windows `service install` 现在先注册并回读验证 SID-scoped 的当前用户 Scheduled Task，再停用 legacy 固定名 task/LocalSystem SCM service；迁移失败会恢复旧运行体或保留已验证的新 task，且绝不覆盖其他 SID 的固定名 task。Windows/macOS/Linux 后台服务都会显式传递所属用户的 Codex/Claude home 路径，且不把 token 放入启动参数。HTTP、SSE、WebSocket、probe、live smoke 和余额刷新现在捕获同一不可变 credential generation；轮换只影响之后的请求，401/403 可触发 refresh 但不会自动重放失败请求。
- HTTP `POST /responses`、`POST /chat/completions`、`POST /messages`、`POST /responses/compact`，以及独立的 Responses WebSocket 与 hosted-image compatibility 流量，保持 economic、route-facing 语义。`/models`、files/uploads/batches/containers 资源族、未知端点及非 POST 的 inference/compact-like HTTP 请求改为 non-economic、request-local：可以在当前请求内 failover，但不会污染共享 cooldown、quota policy、session affinity，也不会因 401/403 调度共享 credential refresh。
- Routing TUI 的 `Enter` 现在打开本机运行时控制菜单：可设置仅影响尚无 affinity 会话的新会话首选，或将 endpoint 设为 Enabled / Draining / Disabled；这些操作不改写用户路由配置，也不迁移已有会话。`a` / `Backspace` / `Delete` 清除新会话首选并恢复容量感知的自动调度。
- Sessions TUI 现在可对空闲且已有 affinity 的会话执行显式 Clear/Rebind。Rebind 只允许迁移到当前 route graph 内、当前可用且具有相同显式 `continuity_domain` 的 endpoint，并由 affinity revision CAS 防止并发请求期间误迁移；远程 observer 保持只读。
- Sessions TUI 恢复 CWD、profile、model、reasoning effort、service tier、缓存读入/新建与命中率，并支持对空闲会话显式设置或重置 profile/model/effort/tier；Settings 恢复 5m/1h 运行摘要、top provider/endpoint、余额、价格目录、最近缓存命中率、本机路径与可滚动布局。Integrated 直接执行本机控制，daemon 同机的 LocalAttached 只显示签名 capability 授权的操作，RemoteObserver 只显示脱敏只读快照。
- 恢复 `switch on/off/status --claude` 和前台 `ch --claude` 兼容路径：Claude 只允许端口/URL 选择，listener 就绪后才临时 patch；前台恢复带 generation lease，只会恢复自身仍拥有的代次，绝不撤销后来的显式或前台选择。原始 settings 与 sidecar 均按私有权限保存；sidecar 只记录指纹、helper target、缺文件标记和无凭据代次，外部编辑与不可验证的旧 backup 会失败关闭而不会被覆盖。
- macOS/Linux service 现在在 install/start/stop/restart/uninstall 前验证 receipt 记录的 executable、完整 plist/unit 和 manager 注册。LaunchAgent 必须来自预期 plist；systemd user unit 必须无 drop-in、无待 reload 且匹配 fragment/enablement。正常 binary relocation 仍由已验证的旧定义过渡到新 receipt，外部同名定义不会被接管。

### English summary

#### Added

- TUI Usage page 5 now puts remote shared-pool used/remaining, 15/60-minute burn, required rate until reset, pace, exhaustion ETA, source/scope/identity confidence, and freshness in the first viewport. Press `g` to reread the operator read model immediately; Projects shows this daemon's Git-project attribution, unknown, external, and signed gap.
...
- Route graphs now support capacity-weighted `round-robin`: cold sessions are distributed smoothly by `max_concurrent_requests - active_requests`, saturated candidates are skipped, and session affinity keeps the selected key after the first success. One key may serve many sessions.
- Restored the complete `[codex.client_patch]` and `switch on --preset ...` surface. Five presets combine `responses_websocket`, `compaction`, `translate_models`, and `hosted_image_generation`, with effective state visible in CLI, TUI, and desktop. `chatgpt-bridge` validates and preserves a complete ChatGPT login; image-generation presets retain a semantic empty `{}` auth facade for Codex versions from the 0.20.3 era, while current Codex image/web extension gating uses `requires_openai_auth = false` plus a non-empty helper actor marker. Original auth and any sensitive headers/comments in an inactive `codex_proxy` stanza live only in private helper backups, the JSON journal stores fingerprints and random filenames, and CAS restores them exactly across switch-off, reapplication, and crash recovery. The helper actor marker is consumed locally and never reaches an upstream.
- The Routing TUI now uses a dense endpoint table suited to long provider lists: wide terminals show a candidate/selected-endpoint master-detail view and narrow terminals fall back to a full-width table. Route order, preference group, endpoint priority, concurrency capacity, balance, and sample age remain visible. Entering the page requests one non-forced refresh. While it remains open, a missing, stale, or roughly six-minute-old sample delegates another daemon-owned refresh no more often than every two minutes. `g` bypasses the UI auto-refresh throttle and forces a full refresh; remote observers remain read-only, and page/edge navigation is available.
- The canonical configuration is now `version = 6`, with typed `native` / `secret_file` bearer and API-key references plus credential CRUD/status and provider-auth binding commands that never accept a value in argv or print it.
- Windows Credential Manager, macOS Keychain, and Linux Secret Service can now serve an installed same-user desktop service. A non-secret receipt, signed loopback refresh, and shared redacted readiness model distinguish `ready`, `degraded`, and `blocked` across CLI, service status, TUI, desktop, and Fleet.
- `codex-helper-server --check [--json]` validates environment and absolute secret-file deployments without opening SQLite, listeners, or upstream connections. Docker Compose adds an optional read-only mounted-secret overlay and real-Engine smoke.
...
- The Codex switch now manages a complete client patch rather than only a URL and reduced capability projection. With no overrides it reads version 6 configuration; all five fields can be overridden with `--preset`, `--compaction`, `--responses-websocket[=false]`, `--translate-models[=false]`, and `--hosted-image-generation`. Old `--mode` gets an explicit migration error. `switch off` restores only helper-owned selector, stanza, and feature keys in the current valid TOML, retaining other provider, feature, project, comment, and runtime edits; managed-field conflicts still fail closed. An auth-bearing patch retains a private recovery point while reporting `Off`, allowing a later semantically equivalent Codex facade write to be repaired; no-replace CAS preserves competing files across crashes and races. A patch can be reapplied safely at the same target; changing the URL still requires switch-off. `switch remote-control`, management of `[features].remote_connections`, and direct Codex App SQLite hacks remain removed. Current `switch on/off` safely recovers verifiable v1/v2 `~/.codex/codex-helper-switch-state.json` left through 0.20.3, while malformed/unknown state and legacy/current conflicts remain unchanged and fail closed.
- Removing the code does not undo Codex `[features].remote_connections = true` or the SQLite `remote_control` row previously written by `switch remote-control enable`. The new release cannot reliably determine whether the user still needs that state, so it does not clean it automatically; do not modify Codex SQLite with an SQL hack.
- The 0.20.3 `state/session-route-affinities.json` ledger is not imported into the new `state.sqlite`, rewritten, or deleted. Finish state-bound sessions that still depend on old multi-endpoint affinity before upgrading, then start a new session; do not try to guess-rebind an old session by sending an ordinary request with the same key because it may select a different endpoint.
- Version 5 is now a one-time migration input. Startup preserves an exact `.bak` and produces the sole version 6 runtime contract. `[codex.client_patch]` is retained while legacy `mode`, underscore/bridge aliases, compaction aliases, and hosted-image aliases normalize to canonical values; the Codex-only `[codex.compaction]` policy is retained and defaults to the v2-to-v1 fallback, including an explicit `false`; only the inapplicable `[claude.compaction]`, usage-forecast, and retired retry/profile/relay-target fields are removed precisely. Existing credential semantics and values are preserved. A v5-only binary rejects v6, so downgrade requires restoring the pre-migration backup. Restoring that backup does not remove native-store entries created under version 6, so orphaned logical names require explicit review.
- Route graphs now support `scheduling_preset = "continuity-first" | "balanced" | "throughput-first"`. Fresh or current version 6 configuration defaults to `balanced`, which waits up to two seconds when the local concurrency cap is saturated. Migration from pre-v6 configuration explicitly writes `throughput-first` when the field is absent, automatically preserving 0.20.3's immediate-failover behavior; an explicit value is never overwritten.
- Persisted route affinity now uses a versioned canonical SHA-256 route key. Changing `scheduling_preset`, adjusting `max_concurrent_requests` / `limit_group`, or editing display-only aliases/metadata no longer invalidates an existing binding, while route rules, endpoint identity, or configured `auth_token` / `api_key` credentials still invalidate it conservatively. Pre-release `state.sqlite` affinities that use the old key format no longer match; start a new session whenever a client-passthrough account or external credential fallback changes.
- `usage_providers.json` retains read-only compatibility with safe v0.20.3 base/time/custom-variable templates and non-sensitive headers. Credentials are rejected in URLs and are allowed only in `Authorization` or `X-API-Key`. An invalid provider is isolated with a diagnostic while other providers continue refreshing, and the source file is never rewritten.
- Startup now migrates v1-v5 and unversioned TOML, plus historical `config.json` when canonical TOML is absent, to version 6; JSON-only `config init` also migrates and prints the report. The complete result is validated and a `.bak` is created before publication; malformed input, future versions, symlinks, or concurrent source changes fail closed. Historical station-shaped JSON accepts only the nullable fields published by the old loader, while `null` in provider-shaped JSON is rejected conservatively. Legacy flat `[retry]` fields move to `[retry.upstream]` only when that table is absent; an existing nested table remains the complete historical override. Use `config migrate --dry-run` for a redacted preview and `config migrate --write --yes` to apply explicitly; clean v6 is a no-op. `config init` no longer performs the legacy full Codex-provider import, and `--no-import` remains removed. A separate first-`ch` onboarding path imports only Codex's currently selected provider when helper has no Codex provider or route, persisting the base URL and credential-reference name but never a credential value. Ambiguous authentication or an unsafe third-party OpenAI-auth origin fails closed. The helper route may remain after startup fails, while the Codex switch is applied only after the runtime is ready.
- Trust and capacity semantics that cannot be inferred safely from 0.20.3 configuration are not rewritten automatically: remote `relay_targets` need an explicit trusted `admin_url`, enabled non-loopback fleet nodes need HTTPS and a valid `admin_token_env`, and every candidate in one explicit `limit_group` needs the same `max_concurrent_requests`. Legacy root `[models.*]` rows in `pricing_overrides.toml` remain readable as OpenAI and normalize to provider-scoped v2 on the first explicit pricing write; rows previously intended for Anthropic/Claude must be moved manually to that provider namespace.
- Remote third-party Codex endpoints without helper credentials are now unroutable by default and return 503 before upstream I/O. Relays that intentionally require no authentication must set `allow_anonymous = true` or use `provider add --allow-anonymous`. This version 5 compatibility rule migrates unchanged to version 6; official OpenAI and loopback behavior is unchanged, and broken explicit credential references never fall back to anonymous access.
- Installed local services, remote hosts, and Docker are placements of one server product. Each receiving server independently owns configuration, runtime store, affinity, round-robin cursor, concurrency, health, and quota; Fleet remains read-only and does not coordinate credentials or capacity across servers.
...
- Route-graph compilation now rejects missing or different `max_concurrent_requests` values within one explicit `limit_group`, so shared-semaphore capacity cannot depend accidentally on candidate order. Invalid version 6 input fails closed without rewriting the source file.
...
- Windows `service install` now registers and reads back a SID-scoped current-user Scheduled Task before retiring a legacy fixed-name task or LocalSystem SCM service. Migration failures restore the old runtime or retain the verified new task, and a fixed-name task owned by another SID is never overwritten. Windows, macOS, and Linux background services pass the owning user's Codex/Claude home path explicitly without putting tokens in launch arguments. HTTP, SSE, WebSocket, probe, live-smoke, and balance-refresh paths now capture one immutable credential generation; rotation affects later requests only, and 401/403 may trigger refresh without replaying the failed request.
- HTTP `POST /responses`, `POST /chat/completions`, `POST /messages`, and `POST /responses/compact`, plus the dedicated Responses WebSocket and hosted-image compatibility flows, retain economic, route-facing semantics. `/models`, the files/uploads/batches/containers resource families, unknown endpoints, and non-POST inference/compact-like HTTP requests are non-economic and request-local: they may fail over within the current request but cannot poison shared cooldown, quota policy, or session affinity, and their 401/403 responses do not schedule shared credential refresh.
- Routing TUI `Enter` now opens local runtime controls for preferring an endpoint only for sessions without affinity, or setting an endpoint to Enabled / Draining / Disabled. These actions do not rewrite user routing configuration or migrate existing sessions. `a` / `Backspace` / `Delete` clears the new-session preference and restores capacity-aware automatic scheduling.
- Sessions TUI can now explicitly Clear/Rebind an idle session that already has affinity. Rebind is limited to a currently available endpoint in the active route graph with the same explicit `continuity_domain`, and affinity-revision CAS prevents migration across a concurrent request; remote observers remain read-only.
- The Sessions TUI again shows CWD, profile, model, reasoning effort, service tier, cache read/create, and hit rate, and can explicitly set or reset profile/model/effort/tier on an idle session. Settings restores 5m/1h runtime summaries, top provider/endpoint, balances, pricing catalog, latest cache hit rate, local paths, and scrolling. Integrated mode executes local controls directly, daemon-host-local LocalAttached mode advertises only signed capabilities, and RemoteObserver displays only a redacted read-only snapshot.
- Restored `switch on/off/status --claude` and foreground `ch --claude` compatibility. Claude accepts only a port or URL and patches only after the listener is ready; foreground restoration carries a generation lease and restores only its still-owned generation, never undoing a later explicit or foreground selection. Original settings and the sidecar use private permissions; the sidecar carries only fingerprints, helper target metadata, an absent-file marker, and a credential-free generation, while external edits or unverifiable legacy backups fail closed.
- macOS/Linux service mutations now verify the receipt-authorized executable, complete plist/unit, and manager registration before install/start/stop/restart/uninstall. A LaunchAgent must originate from the expected plist; a systemd user unit must have no drop-in or pending reload and match its fragment/enablement. A normal binary relocation still moves from a verified old definition to a new receipt, while an external same-name definition is never adopted.
...
- 桌面端 `Usage` 页改为今日数据面板，展示今日请求、token、估算成本、24 小时活跃度、provider / model / project 排行、日志覆盖提示和全局 retry gate 数量。
- 新增 request chain 诊断。可以用 `codex-helper usage chain --trace-id ... --json` 或本地 Admin API 查看单次请求的 route attempts、provider signals、policy actions 和时间线；桌面端 `Usage` 表格也支持展开 Chain。
- 桌面端 Providers 页面现在会显示正在影响路由的 provider control badges，并在页面顶部汇总当前 active control 数量。

#### 变更

- request chain、provider signals 和 policy actions 补充稳定 code，脚本和桌面端不再需要解析展示文案来判断路由控制原因。
- 本地 Admin/runtime 错误改为结构化返回，桌面端和脚本更容易区分连接失败、权限失败、状态冲突和响应解析失败。

#### 修复

- 修复日套餐耗尽后的跨天余额恢复。每日重置时间到达后不再沿用昨天的余额抑制；`g` / 桌面刷新可以清理跨天遗留的临时抑制，但仍会避免重复请求今天已经确认耗尽的套餐。
...
- The desktop `Usage` page now uses the local-day usage model: today's requests, tokens, estimated cost, 24h activity, provider / model / project rankings, coverage warnings, and the global retry-gate count.
- Added request-chain diagnostics. Use `codex-helper usage chain --trace-id ... --json` or the local Admin API to inspect route attempts, provider signals, policy actions, and the request timeline; the desktop `Usage` table can expand Chain details per row.
- Desktop Providers now shows route-affecting provider control badges and a page-level active-control count.

#### Changed

- Request chains, provider signals, and policy actions now include stable codes so scripts and the desktop app do not need to parse display text.
- Local Admin/runtime errors now return structured information, making connection, authorization, state-conflict, and response-decoding failures easier to distinguish.

#### Fixed

- Fixed daily-package balance recovery after reset. Once the configured daily reset time passes, yesterday's exhausted-balance suppression no longer blocks polling; `g` / desktop refresh can clear orphaned cross-day suppression while still avoiding repeated calls for packages already confirmed exhausted today.
