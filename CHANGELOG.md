...
- 完整恢复 `[codex.client_patch]` 与 `switch on --preset ...`：五种 preset 可组合 `responses_websocket`、`compaction`、`translate_models` 和 `hosted_image_generation`，CLI/TUI/桌面端均显示生效状态。`chatgpt-bridge` 会验证并保留完整 ChatGPT 登录；imagegen preset 可按 Codex gating 临时呈现语义空 `{}` auth facade。原始 auth 只存放在私有 helper backup，JSON journal 只保存指纹和随机文件名，CAS 可在关闭、重复应用与崩溃恢复时精确还原；helper actor marker 不会上送。
- Routing TUI 改为适合长 provider 列表的密集 endpoint 表：宽屏使用候选列表/选中端点详情主从布局，窄屏退化为全宽表；同时展示路由顺序、偏好组、endpoint priority、并发容量和余额。进入页面自动请求一次 daemon-owned 刷新，`g` 强制全量刷新，并支持分页与首尾导航。
...
- Codex switch 现在管理完整 client patch，而不是仅管理 URL/facade：不带覆盖时读取 version 6 配置，五项字段可由 `--preset`、`--compaction`、`--responses-websocket[=false]`、`--translate-models[=false]` 和 `--hosted-image-generation` 临时覆盖；`--client-facade` 保留为兼容快捷方式，旧 `--mode` 会给出明确迁移提示。认证型 patch 关闭后保留私有恢复点并显示 `Off`，可修复 Codex 稍后重写的语义等价 facade；no-replace CAS 在崩溃和竞争写入时不会覆盖外部文件。相同 target 可安全重复应用不同 patch；更换 URL 仍需先关闭。`switch remote-control`、`[features].remote_connections` 管理和直接读写 Codex App SQLite 的 hack 保持删除。0.20.3 及更早版本留下的 v1/v2 `~/.codex/codex-helper-switch-state.json` 会在旧 patch 仍可验证时由 `switch on/off` 自动恢复；损坏、未知版本或新旧 journal 冲突会原样保留并失败关闭。
...
- Version 5 现在是一次性迁移输入：启动先保留精确 `.bak`，再生成唯一运行时契约 version 6。`[codex.client_patch]` 会保留并把旧 `mode`、underscore/bridge alias、compaction 与 hosted-image alias 规范化为 canonical 值；独立 `[codex.compaction]` / `[claude.compaction]`、`[ui.usage_forecast]` 和退休 retry/profile/relay-target 字段仍会精确清理。迁移保留已有凭据语义和值；只懂 v5 的旧 binary 会拒绝 v6，降级必须恢复迁移前备份。恢复旧备份不会删除 version 6 创建的 native-store entry，因此必须显式检查孤立逻辑名。
- Route graph 新增 `scheduling_preset = "continuity-first" | "balanced" | "throughput-first"`，缺省 `balanced` 会在本地并发上限饱和时最多等待 2 秒。要保留 0.20.3 的立即 failover 行为，应显式使用 `throughput-first`。
...
- `usage_providers.json` 现在严格拒绝未知字段和 endpoint 模板。旧 `headers` / `variables` / `{{...}}` 写法必须迁移为字面量 relative/absolute endpoint；`new_api_user_self` 的 `New-Api-User` 身份改用 `new_api_user_id_env`。
- 启动现在会把 v1-v5、无版本 TOML，以及缺少 canonical TOML 时的历史 `config.json` 自动迁移到 version 6；JSON-only `config init` 也会迁移并输出报告。写入前会验证完整结果并创建 `.bak`，损坏输入、未来版本、符号链接或并发源文件变化会 fail closed。历史 station-shaped JSON 只接受旧 loader 正式支持的 nullable 字段，provider-shaped JSON 中的 `null` 保守拒绝。只有 `[retry.upstream]` 不存在时，旧 `[retry]` flat 字段才会迁入该表；已存在的 nested 表会作为完整历史 override 保留。`config migrate --dry-run` 可脱敏预览，`config migrate --write --yes` 可显式执行；干净 v6 是 no-op。Codex provider 自动导入和 `config init --no-import` 仍保持移除。
...
- `/models` 的失败、翻译与 request-local failover 不再打开或清除对话/compact 共用的 cooldown、quota policy 或 session affinity；只有实际 inference 请求能改变共享 route health。
...
- Restored the complete `[codex.client_patch]` and `switch on --preset ...` surface. Five presets combine `responses_websocket`, `compaction`, `translate_models`, and `hosted_image_generation`, with effective state visible in CLI, TUI, and desktop. `chatgpt-bridge` validates and preserves a complete ChatGPT login; image-generation presets may present a semantic empty `{}` auth facade for Codex gating. Original auth lives only in a private helper backup, the JSON journal stores fingerprints and a random filename, and CAS restores exact bytes across switch-off, reapplication, and crash recovery. The helper actor marker never reaches an upstream.
- The Routing TUI now uses a dense endpoint table suited to long provider lists: wide terminals show a candidate/selected-endpoint master-detail view and narrow terminals fall back to a full-width table. Route order, preference group, endpoint priority, concurrency capacity, and balance remain visible. Entering the page requests one daemon-owned refresh, `g` forces a full refresh, and page/edge navigation is available.
...
- Version 5 is now a one-time migration input. Startup preserves an exact `.bak` and produces the sole version 6 runtime contract. `[codex.client_patch]` is retained while legacy `mode`, underscore/bridge aliases, compaction aliases, and hosted-image aliases normalize to canonical values; standalone `[codex.compaction]` / `[claude.compaction]`, usage-forecast, and retired retry/profile/relay-target fields are still removed precisely. Existing credential semantics and values are preserved. A v5-only binary rejects v6, so downgrade requires restoring the pre-migration backup. Restoring that backup does not remove native-store entries created under version 6, so orphaned logical names require explicit review.
- Route graphs now support `scheduling_preset = "continuity-first" | "balanced" | "throughput-first"`. The default `balanced` waits up to two seconds when the local concurrency cap is saturated; use `throughput-first` to preserve 0.20.3's immediate-failover behavior.
...
- `/models` failures, translation, and request-local failover no longer open or clear inference/compact cooldown, quota policy, or session affinity. Shared route health changes only from actual inference traffic.
