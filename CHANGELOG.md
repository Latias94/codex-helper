...
- Linux CI/cargo-dist dependencies now include the WebKitGTK 4.1 / JavaScriptCoreGTK 4.1 development packages required by Tauri v2, avoiding `javascriptcoregtk-4.1.pc` build failures during clippy/build. Linux CI also now prebuilds and stages the Tauri Linux sidecar before `cargo clippy` / `cargo build`, so `tauri_build` no longer aborts on a missing `externalBin` artifact.
