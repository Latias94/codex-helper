...
- TUI `Stations` 页新增 v3 routing 编辑器：支持 provider pin、顺序兜底重排、包月标签优先、`on_exhausted` 切换，以及常用 `billing=monthly/paygo` tag 快捷编辑。 TUI `Stations` adds a v3 routing editor for provider pins, ordered failover reordering, monthly tag preference, `on_exhausted` toggling, and quick `billing=monthly/paygo` tag edits.
