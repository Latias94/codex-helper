...
- Codex `/responses/compact` 现在会先尝试已存在的 session affinity；如果 affinity 账号不可用且 route graph 不是 `Hard`，会按路由策略回退到同池其它可用账号。`Hard` 仍保持严格单账号粘性，相关 compact 回归测试也已补齐。
- Codex `/responses/compact` 现在会保留官方 compaction 输入里的 `service_tier` 和 `prompt_cache_key`，并且如果请求体里带有 `encrypted_content` 或 `previous_response_id` 这类状态字段，就会把 compact 视为 state-bound 请求，避免跨账号兜底把 502 误变成上游状态错误。
...
- Codex `/responses/compact` now tries the existing session affinity first. If that affinity account is unavailable and the route graph is not `Hard`, it falls back to another available account in the same pool according to routing policy. `Hard` remains strict single-account stickiness, with compact regression coverage added for the new behavior.
- Codex `/responses/compact` now preserves the official input fields `service_tier` and `prompt_cache_key`, and treats requests carrying `encrypted_content` or `previous_response_id` as state-bound so compact fallback does not cross accounts when upstream state might be pinned to one relay identity.
