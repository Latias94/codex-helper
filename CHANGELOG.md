...
- `switch on --client-facade compatible|openai|openai-tools` 重新提供显式 Codex 客户端能力声明：`openai` 写入官方 provider identity 以允许 remote compact / Web Search，`openai-tools` 再用当前 Codex 支持的 provider header contract 允许 hosted image generation。默认保持 `compatible`；不写空 `{}` `auth.json`，helper marker 不会上送，真实 actor authorization 只可能透传到未配置 helper 凭据的 OpenAI 官方源站。
...
- Codex switch surface 收敛为 `switch on [--port | --base-url] [--client-facade ...]`、`switch off` 和 `switch status`，只管理 helper 自有的 provider selector/stanza。`switch remote-control`、preset/mode/compaction/WebSocket 等旧客户端假设参数、`[codex.client_patch]`、`[features].remote_connections` 和直接读写 Codex App SQLite 的 hack 均已删除；facade 选择作为小型显式契约直接记录进 switch journal，更换 facade 必须先 `switch off`。下方旧版本中关于已删除接口的条目仅保留为历史记录，已被本项取代。0.20.3 及更早版本留下的 v1/v2 `~/.codex/codex-helper-switch-state.json` 现由新版 `switch on/off` 在旧 patch 仍可验证时自动安全恢复；损坏、未知版本或新旧 journal 冲突会原样保留并失败关闭。该文件可能包含原始 auth，恢复前不得删除、编辑或分享。
...
- Version 5 现在是一次性迁移输入：启动先保留精确 `.bak`，再生成唯一运行时契约 version 6，并精确清理 `[codex.client_patch]`、`[codex.compaction]`、`[claude.compaction]`、`[ui.usage_forecast]`、退休 retry/profile/relay-target 字段。迁移保留已有 inline/environment/client-file 凭据语义，不会生成引用、导入或删除凭据值；只懂 v5 的旧 binary 会拒绝 v6，降级必须恢复迁移前备份。只有在尚未进行 v6-only provider/reference 修改时回滚才完全可逆；之后恢复旧备份会丢弃这些配置改动，但不会删除 native-store entry，必须人工检查孤立逻辑名。
...
- `switch on --client-facade compatible|openai|openai-tools` restores an explicit Codex client-capability advertisement. `openai` writes the official provider identity for remote compaction and Web Search eligibility; `openai-tools` additionally uses the current Codex provider-header contract for hosted image generation eligibility. The default remains `compatible`; no empty `{}` `auth.json` is written, the helper marker never reaches an upstream, and real actor authorization can pass only to an official OpenAI origin without configured helper credentials.
...
- The Codex switch surface is now limited to `switch on [--port | --base-url] [--client-facade ...]`, `switch off`, and `switch status`, and manages only the helper-owned provider selector/stanza. `switch remote-control`, the old preset/mode/compaction/WebSocket flags, `[codex.client_patch]`, `[features].remote_connections`, and the direct Codex App SQLite hack have been removed. Facade selection is a small explicit contract recorded directly in the switch journal, and changing it requires `switch off` first. Older entries below that mention removed interfaces remain historical and are superseded by this change. Current `switch on/off` safely recovers v1/v2 `~/.codex/codex-helper-switch-state.json` left by releases through 0.20.3 while the old patch remains verifiable; malformed or unknown state and legacy/current journal conflicts are preserved unchanged and fail closed. The legacy file may contain original auth content and must not be deleted, edited, or shared before recovery.
...
- Version 5 is now a one-time migration input. Startup preserves an exact `.bak`, produces the sole version 6 runtime contract, and precisely removes retired client-patch, compaction, usage-forecast, retry/profile, and relay-target fields. Existing inline/environment/client-file auth semantics are preserved without synthesizing references or importing/deleting values. A v5-only binary rejects v6; downgrade requires restoring the pre-migration backup. Rollback is fully reversible only before v6-only provider/reference changes; restoring the old backup later discards those config edits but does not delete native-store entries, so orphaned logical names require explicit review.
