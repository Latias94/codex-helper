...
- 0.20.3 的 `state/session-route-affinities.json` 不会导入新的 `state.sqlite`，也不会被自动改写或删除。升级后继续旧的多 endpoint 会话时，应先用同一 session key 完成一次普通 Responses 请求以重新建立 affinity，再发送依赖上游状态的 compact 请求。
...
- The 0.20.3 `state/session-route-affinities.json` ledger is not imported into the new `state.sqlite`, rewritten, or deleted. Before continuing an older multi-endpoint session after upgrading, complete one ordinary Responses request with the same session key to re-establish affinity before sending state-bound compact requests.
