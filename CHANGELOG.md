...
- 同一个 `version = 5` 契约中的可选配置已收敛：启动会在改写前聚合拒绝 `[codex.client_patch]`、`[codex.compaction]`、旧共享 schema 接受但从未影响 Claude runtime 的 `[claude.compaction]`、`[ui.usage_forecast]`、`[retry].allow_cross_station_before_first_output`、profile `station` 与 relay target `client_preset` / `responses_websocket`；server config 同样拒绝 `advertised-admin-base-url` / `host-local-session-history`。普通 provider/endpoint/route graph 仍兼容，源文件不会被静默改写。
...
- 持久 route affinity 改用版本化的 canonical SHA-256 route key；切换 `scheduling_preset` 或纯展示 alias/metadata 不再废掉已有 binding，而 route rule、endpoint identity 或配置内 `auth_token` / `api_key` 凭据变化仍会使旧 binding 保守失效。预发布 `state.sqlite` 中使用旧 key 格式的 affinity 不再匹配；客户端透传账号或外部 credentials fallback 发生变化时必须开启新会话。
...
- `config migrate`、Codex provider 自动导入以及 `config init --no-import` 已移除。历史 schema 不再由 runtime 兼容读取；请用 `config init` 创建独立的当前配置，并人工表达 provider/routing intent。
...
- Optional inputs published under the same `version = 5` contract have been narrowed. Before any rewrite, startup now rejects and reports `[codex.client_patch]`, `[codex.compaction]`, `[claude.compaction]` (accepted by the old shared schema but never used by the Claude runtime), `[ui.usage_forecast]`, `[retry].allow_cross_station_before_first_output`, profile `station`, and relay-target `client_preset` / `responses_websocket`; server config likewise rejects `advertised-admin-base-url` / `host-local-session-history`. Normal provider/endpoint/route graphs remain compatible, and source files are not silently rewritten.
...
- Persisted route affinity now uses a versioned canonical SHA-256 route key. Changing `scheduling_preset` or display-only aliases/metadata no longer invalidates an existing binding, while route rules, endpoint identity, or configured `auth_token` / `api_key` credentials still invalidate it conservatively. Pre-release `state.sqlite` affinities that use the old key format no longer match; start a new session whenever a client-passthrough account or external credential fallback changes.
...
- `config migrate`, automatic Codex-provider import, and `config init --no-import` have been removed. Historical schemas no longer have a compatibility runtime reader; create a separate current config with `config init` and express provider/routing intent manually.
