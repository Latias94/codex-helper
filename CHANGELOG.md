...
- Codex switch surface 收敛为 `switch on [--port | --base-url]`、`switch off` 和 `switch status`，只管理 helper 自有的 provider selector/stanza。`switch remote-control`、preset/mode/compaction/WebSocket 等客户端假设参数、`[codex.client_patch]`、`[features].remote_connections` 和直接读写 Codex App SQLite 的 hack 均已删除；下方旧版本中关于这些接口的条目仅保留为历史记录，已被本项取代。0.20.3 及更早版本留下的 v1/v2 `~/.codex/codex-helper-switch-state.json` 现由新版 `switch on/off` 在旧 patch 仍可验证时自动安全恢复；损坏、未知版本或新旧 journal 冲突会原样保留并失败关闭。该文件可能包含原始 auth，恢复前不得删除、编辑或分享。
...
- The Codex switch surface is now limited to `switch on [--port | --base-url]`, `switch off`, and `switch status`, and manages only the helper-owned provider selector/stanza. `switch remote-control`, client-assumption flags for presets/modes/compaction/WebSocket, `[codex.client_patch]`, `[features].remote_connections`, and the direct Codex App SQLite hack have been removed. Older entries below that mention those interfaces remain as historical release records and are superseded by this change. Current `switch on/off` safely recovers v1/v2 `~/.codex/codex-helper-switch-state.json` left by releases through 0.20.3 while the old patch remains verifiable; malformed or unknown state and legacy/current journal conflicts are preserved unchanged and fail closed. The legacy file may contain original auth content and must not be deleted, edited, or shared before recovery.
