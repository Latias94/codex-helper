...
- 同一个 `version = 5` 契约中的可选配置已收敛：启动会先备份 `config.toml`，再精确清理 `[codex.client_patch]`、`[codex.compaction]`、旧共享 schema 接受但从未影响 Claude runtime 的 `[claude.compaction]`、`[ui.usage_forecast]`、`[retry].allow_cross_station_before_first_output`、profile `station` 与 relay target `client_preset` / `responses_websocket`；typed save 仍拒绝直接覆盖尚未清理的源文件。server config 同样拒绝 `advertised-admin-base-url` / `host-local-session-history`，普通 provider/endpoint/route graph 仍兼容。
...
- 启动现在会把 v1-v4、无版本 TOML，以及缺少 canonical TOML 时的历史 `config.json` 自动迁移到 version 5；JSON-only `config init` 也会迁移并输出报告。写入前会验证完整结果并创建 `.bak`，损坏输入、未来版本、符号链接或并发源文件变化会 fail closed。历史 station-shaped JSON 只接受旧 loader 正式支持的 nullable 字段，provider-shaped JSON 中的 `null` 保守拒绝。只有 `[retry.upstream]` 不存在时，旧 `[retry]` flat 字段才会迁入该表；已存在的 nested 表会作为完整历史 override 保留。`config migrate --dry-run` 可脱敏预览，`config migrate --write --yes` 可显式执行或清理当前 v5 的退休字段；干净 v5 是 no-op。Codex provider 自动导入和 `config init --no-import` 仍保持移除。
...
- 远程第三方 Codex endpoint 在未配置 helper 凭据时默认不可路由，并在任何上游 I/O 前返回 503；确实不要求认证的 relay 必须显式设置 `allow_anonymous = true` 或使用 `provider add --allow-anonymous`。这是 version 5 内的兼容字段扩展，不重写旧配置；官方 OpenAI 与 loopback 行为不变，失效的显式凭据引用也不会回退匿名。
...
- Route graph 编译现在拒绝同一显式 `limit_group` 中缺失或不同的 `max_concurrent_requests`，避免共享 semaphore 的容量由候选顺序偶然决定；无效 version 5 配置会失败关闭且不改写源文件。
...
- Windows `service install` 现在先注册并回读验证 SID-scoped 的当前用户 Scheduled Task，再停用 legacy 固定名 task/LocalSystem SCM service；迁移失败会恢复旧运行体或保留已验证的新 task，且绝不覆盖其他 SID 的固定名 task。Windows/macOS/Linux 后台服务都会显式传递所属用户的 Codex/Claude home 路径，且不把 token 放入启动参数。HTTP、WebSocket、probe、live smoke 和余额刷新共用 inline → 进程环境 → 显式同名客户端字段的凭据 resolver，未解析的显式引用会在访问上游前失败关闭，避免后台进程把无 Authorization 请求发给 sub2api 类中转。
...
- Optional inputs published under the same `version = 5` contract have been narrowed. Startup backs up `config.toml` and then precisely removes `[codex.client_patch]`, `[codex.compaction]`, `[claude.compaction]` (accepted by the old shared schema but never used by the Claude runtime), `[ui.usage_forecast]`, `[retry].allow_cross_station_before_first_output`, profile `station`, and relay-target `client_preset` / `responses_websocket`; typed saves still refuse to overwrite an uncleaned source directly. Server config likewise rejects `advertised-admin-base-url` / `host-local-session-history`, while normal provider/endpoint/route graphs remain compatible.
...
- Startup now migrates v1-v4 and unversioned TOML, plus historical `config.json` when canonical TOML is absent, to version 5; JSON-only `config init` also migrates and prints the report. The complete result is validated and a `.bak` is created before publication; malformed input, future versions, symlinks, or concurrent source changes fail closed. Historical station-shaped JSON accepts only the nullable fields published by the old loader, while `null` in provider-shaped JSON is rejected conservatively. Legacy flat `[retry]` fields move to `[retry.upstream]` only when that table is absent; an existing nested table remains the complete historical override. Use `config migrate --dry-run` for a redacted preview and `config migrate --write --yes` to apply explicitly or clean retired fields from current v5 input; clean v5 is a no-op. Automatic Codex-provider import and `config init --no-import` remain removed.
...
- Remote third-party Codex endpoints without helper credentials are now unroutable by default and return 503 before upstream I/O. Relays that intentionally require no authentication must set `allow_anonymous = true` or use `provider add --allow-anonymous`. This is a compatible version 5 field addition and does not rewrite old files; official OpenAI and loopback behavior is unchanged, and broken explicit credential references never fall back to anonymous access.
...
- Route-graph compilation now rejects missing or different `max_concurrent_requests` values within one explicit `limit_group`, so shared-semaphore capacity cannot depend accidentally on candidate order. Invalid version 5 input fails closed without rewriting the source file.
...
- Windows `service install` now registers and reads back a SID-scoped current-user Scheduled Task before retiring a legacy fixed-name task or LocalSystem SCM service. Migration failures restore the old runtime or retain the verified new task, and a fixed-name task owned by another SID is never overwritten. Windows, macOS, and Linux background services pass the owning user's Codex/Claude home path explicitly without putting tokens in launch arguments. HTTP, WebSocket, probe, live-smoke, and balance-refresh paths share an inline → process environment → explicitly referenced same-name client-field credential resolver, and unresolved explicit references fail closed before upstream I/O instead of sending unauthenticated requests to sub2api-style relays.
