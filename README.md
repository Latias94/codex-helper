...
- **显式安全 Switch**：只在人工执行 `switch on/off` 时修改 `~/.codex/config.toml` 的 helper provider selector/stanza；可用 `--client-facade compatible|openai|openai-tools` 显式声明客户端能力，外部编辑冲突会进入 `recovery_required`，Codex auth、模型缓存和 SQLite 永远不属于 helper 所有权。
...
客户端 switch 负责把 Codex 指向一个 helper URL，并可通过 `--client-facade` 显式选择 `compatible`（默认）、`openai`（remote compact + Web Search 候选）或 `openai-tools`（再增加 hosted image generation 候选）。Facade 只控制 Codex 客户端暴露能力，不保证 relay 支持对应请求。`switch on` 记录原 selector 和 helper stanza，只写入 `model_providers.codex_proxy`；`switch off` 只恢复记录过的内容。外部编辑发生冲突时，状态进入 `recovery_required` 并保持文件不动。更换 URL 或 facade 前必须先 `switch off`。除下述一次性 legacy 恢复外，Codex `auth.json`、`models_cache.json`、SQLite、全局 feature flags、compaction 和 WebSocket 设置都不会被读取或修改；历史空 `{}` auth facade 不会恢复。helper 的本地能力 marker 不会上送，真实 actor authorization 只可能透传到未配置 helper 凭据的 OpenAI 官方源站。
