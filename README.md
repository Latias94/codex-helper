...
- 在修改前检查 `~/.codex/config.toml`，如已指向本地代理且存在 codex-helper switch state，会询问是否先关闭旧代理 patch；
- 必要时局部修改 `model_provider` 与 `model_providers.codex_proxy`，让 Codex 走本地代理；退出时只撤销这块 patch，保留 Codex 运行期间写入的其它配置（例如项目 trust）；
...
- 用 Ctrl+C 或在 TUI 中按 `q` 退出时，尝试关闭本地代理 patch，让 Codex 回到启动前的 provider。
...
- 关闭本地代理 patch（不覆盖 Codex 运行期间写入的其它配置）：
