...
- Provider balance adapters can now opt out of routing trust for exhausted snapshots when a source is known to return misleading zero balances, while still surfacing the raw exhausted state in UI and traces.
