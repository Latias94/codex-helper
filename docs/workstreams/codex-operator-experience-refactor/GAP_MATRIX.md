...
| Model price sync | CLI can sync remote `ModelPriceCatalogSnapshot` JSON into local overrides; GUI can save catalog rows as local overrides; no scheduled source cache yet | Seeded pricing table | External sync + cache | Add scheduled source-backed catalog cache later | P1 |
...
| Long-horizon audit | JSONL + runtime rollups + core `request_ledger` query API powering `usage find`, grouped `usage summary --by ...`, and GUI Requests ledger mode over local file or attached admin API | SQLite | SQLite + charts | Add SQLite only as a rebuildable index/cache after v2 schema | P2 |
...
- long-horizon cost audit still depends on runtime rollups and JSONL replay, but JSONL query semantics are now centralized in core `request_ledger` and exposed to attached GUI clients through an admin-protected API
