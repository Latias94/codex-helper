...
The current runtime path still compiles that graph through the legacy v2 shape
and then into a single `routing` station. That keeps compatibility working, but
it also loses important runtime semantics:
