...
The workstream is open. No code has been committed for live smoke yet. Capability diagnostics remain validation-only and should not be changed into live checks.
...
- Implement RLS-030 admin API surface using `CodexRelayLiveSmokeRequest` / `CodexRelayLiveSmokeResponse`.
