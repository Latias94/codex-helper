...
`codex-helper` container deployments are central relay runtimes, not client-configuration owners. Container and server entrypoints never read or write Codex-owned `config.toml`, `auth.json`, model cache, SQLite files, or Claude settings, and they do not provide client-local transcript/session capabilities. Local `session` commands read only the Codex session files on the machine where the command runs. The separate local CLI may explicitly manage a bounded client surface: the helper provider selector/stanza and recorded client-patch fields in Codex `config.toml`, plus Claude `env.ANTHROPIC_BASE_URL` through `switch ... --claude`. Each path requires a human switch operation or the documented foreground `ch` compatibility lease; it uses its own CAS-backed recovery material and never occurs while selecting or attaching to a relay target. This keeps LAN/Tailscale relay behavior honest for devices whose client files live on their own machines.
