...
- Goal: Reduce steady-state memory use in `codex-helper` TUI / dashboard, with emphasis on recent-request retention and branch cache defaults.
- Branch: `main`
- Last verified: `cargo fmt --all`, `cargo check -p codex-helper-core -p codex-helper-tui`, and focused `cargo nextest` passed.
- Done: `recent_finished_max()` now defaults to 1000 and clamps to `200..10000`; dashboard snapshot no longer forces `recent_limit` up to 2000.
