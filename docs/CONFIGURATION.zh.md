...
- 提交后的请求日志：`~/.codex-helper/logs/requests.jsonl`
- 可选的完整 HTTP 调试日志：`~/.codex-helper/logs/requests_debug.jsonl`
...
只有显式执行本地 `switch on/off`，或调用有明确自动切换契约的 `ch` / `ch relay` 兼容流程，才会 patch `~/.codex/config.toml` 中 journal 记录的 provider selector、`model_providers.codex_proxy` 和 client capability fields；所有路径共用同一 journal/CAS 恢复机制。需要 auth facade 的 preset 还会临时 patch `auth.json`，并从 helper 私有 backup 按原始字节恢复。普通 `codex-helper` runtime、TUI 刷新、server 与 Docker 不修改客户端文件。codex-helper 永远不读写 Codex 模型缓存或 SQLite。普通 runtime 不写入 `auth.json`；但当 provider 显式配置 `auth_token_env` / `api_key_env` 而当前进程环境缺少该变量时，Codex runtime 可只读查找 `$CODEX_HOME/auth.json` 顶层的同名字符串字段。该凭据 fallback 与 client patch facade 是两个独立生命周期。
...
`local` 是内置 target，会按当前 `default_service` 解析到普通 loopback 端口；`ch relay local` 使用与普通 `ch` 相同的自动切换流程。对 Codex 命名 target，`ch relay nas` 先 journaled-switch 到 `proxy_url`，再打开远端只读 TUI；`--no-tui` 只执行客户端切换，`--attach-only` 只附着观察且绝不修改客户端。`ch relay off` 恢复 journal 记录的本机配置。Claude target 没有 Codex 客户端切换动作，远端 Claude 的 `--no-tui` 因没有可执行动作而被拒绝。

命名 Codex target 可以包含部分 `[relay_targets.<name>.client_patch]`。其中出现的字段覆盖全局 `[codex.client_patch]` 对应字段，省略字段继续继承全局值。`relay add` 也接受 `--preset`、`--responses-websocket[=false]`、`--compaction`、`--translate-models[=false]` 和 `--hosted-image-generation` 来写入同一份部分契约；Claude target 会拒绝这些 Codex 专用选项。
...
`codex-helper switch` 是显式客户端切换接口；`ch` 与 `ch relay` 是为兼容短命令保留的声明式自动切换入口。Server、普通 `codex-helper` runtime、TUI 刷新、桌面端动作和 capability 结果都不会自行修改 Codex 配置。`ch` 新启前台 runtime 时持有临时 switch lease，并在 runtime 停止后通过 CAS 恢复 helper 管理的投影，同时保留其它有效运行期编辑；附着已验证 native service 或使用 resident 模式时保留持久 switch。`ch relay <codex-target>` 持久切换，`--attach-only` 不切换，`ch relay off` 恢复。所有写入使用下述同一 journal。
...
相同 target URL 下可以重复执行 `switch on` 更换 client patch：helper 会先从当前 config 合并移除旧 journal 的受管投影并恢复记录的 auth facade，再在该结果上应用新 patch；其它有效 config 编辑会继续保留。更换 target URL 仍需先运行 `switch off`。`switch status` 会显示生效的五项 client-patch 字段。

`chatgpt-bridge` 和两种 imagegen preset 都会生成 auth facade。ChatGPT facade 要求原文件包含完整登录材料并保留 token；即使关闭 hosted image generation，imagegen facade 仍会临时写入语义空 `{}`。Hosted-image mode 只控制 feature bit、actor marker 和请求过滤，不改变 preset 的 auth facade。原始 auth 字节存放在私有 backup，而非 JSON journal；`switch off` 通过 CAS 精确恢复。Switch 永远不会读写 `models_cache.json` 或 Codex SQLite，也不会恢复旧 SQL hack。真实上游能力仍来自选中 provider 的契约和实时观测。
...
共享 route state 按 capability 隔离。HTTP `POST /responses`、`POST /chat/completions`、`POST /messages` 和 `POST /responses/compact` 属于 route-facing、economic 流量；Responses WebSocket 保留独立的 route-facing/economic 路径，hosted-image compatibility 请求在内部转换为 Responses 后也保持 route-facing/economic。`/models`、files/uploads/batches/containers 资源族、未知端点，以及对 inference/compact-like HTTP 路径的非 POST 请求都属于 request-local、non-economic。它们不会打开或清除共享 cooldown、quota policy 或 session affinity，其 401/403 也不会调度共享 credential refresh。安全的 request-local `GET`/`HEAD` 读取可以尝试下一个 candidate，包括凭据形状的拒绝；有副作用的 request-local 请求一旦 dispatch，就不会重试或发送给另一 provider，因为上游可能已经执行。Route-facing 重试保持配置语义，但 route-facing 401/403 只为后续请求安排 refresh，不重放本次失败。Request-local 请求体按原始字节透传，不运行 client patch、model mapping、reasoning 或 `filter.json` 转换；model-list 响应翻译仍是 request-local 兼容转换，不是 route-health signal。
...
0.20.3 及更早版本在本地并发上限饱和时会立即 failover。从 pre-v6 配置迁移且该字段缺失时会写入 `scheduling_preset = "throughput-first"`，包括仅配置 providers、依赖隐式 routing 的情况；已有显式 preset 保持不变。新建及当前 version 6 route graph 缺少该字段时使用 `balanced`；若在 version 6 中新写配置且需要立即 failover，应显式设置 `throughput-first`。饱和不会记为 provider 失败，不会打开 cooldown，也不会污染 session affinity。如果应用所选等待策略后所有候选仍然饱和或不可用，请求会走正常的 route-unavailable 路径，而不是凭空造一个 provider。对于共用同一上游账号的多个 endpoint，请给它们设置相同的 `limit_group`，让 runtime 把它们当成一个并发池。
...
`service install --no-start` 只能验证 installer 进程上下文；在执行 `service start` 或 `service restart` 之前，它会明确把已安装 service 上下文标记为 unverified。若同 identity 替换会停止一个正在运行的 Codex service，`--no-start` 会先使用同一套 target-aware CAS 契约恢复匹配的 helper-managed client switch；稍后启动 service 不会隐式把 Codex 再切回来。`service status` 将 OS state 与 `receipt_state`、`credential_context`、`runtime_identity_verified` 分开显示；即使进程已停止、receipt 缺失/旧版/未来版本/损坏或 admin endpoint 不可达，JSON 仍可用。
...
`service uninstall --keep-running` 是显式的 detach 操作：它移除注册、definition 和 receipt，但不停止当前进程，也不修改 Codex switch。detached 进程只会运行到自行退出；此后 service 命令无法再管理它，未来要恢复托管必须重新执行 `service install`。如果 Codex 仍指向该进程，operator 必须在 detached runtime 退出前后自行执行 `switch off`。Windows 的 Scheduled Task 删除不会结束已经运行的 action；正在运行的 legacy SCM service 在删除等待期间也会继续运行。若最终 SCM 删除结果无法确认，命令会明确报告 partial installation 并要求重装/修复，不会误报为完整回滚。
...
| `imagegen-bridge` | `codex-helper` | 临时使用语义空 `{}` | 使用 helper identity 暴露 hosted `image_generation` |
| `official-relay` | `OpenAI` | 保持原样 | 暴露 Web Search / remote compact 等官方 identity gating |
| `official-imagegen` | `OpenAI` | 临时使用语义空 `{}` | 官方 identity 加 hosted `image_generation` gating |
...
- `hosted_image_generation = "auto" | "enabled" | "disabled"` 与 `preset` 正交，分别保留、开启或关闭请求的 Codex feature，这与 v0.20.3 配置契约一致。Preset 仍独立决定 provider identity 与 auth facade，因此 `enabled` 不代表某个 Codex 版本或账号已经通过其自身 image runtime authorization gate。对 image preset，`disabled` 仍保留该 preset 的 provider identity 与语义空 `{}` auth facade，但会关闭 image feature/actor marker，并过滤 Codex 客户端发出的 hosted image tool；helper 的 OpenAI Images 兼容入口仍可独立使用。

`codex-helper switch on` 不带 client-patch 参数时加载这里的全部配置。`--preset` 临时替换 preset，并把该次 switch 的 compaction/WebSocket 恢复为各自默认值；`translate_models` 与 hosted-image 模式保持配置值。五项都可显式覆盖：`--compaction ...`、`--responses-websocket[=true|false]`、`--translate-models[=true|false]` 和 `--hosted-image-generation auto|enabled|disabled`。桌面端可以显式设置全部五项；integrated TUI 的 preset 快捷键使用对应 preset 默认值，配置模式则加载本表。
...
大多数 relay 用户不需要为了显示余额手写 `usage_providers.json`。这个文件是可选且由 operator 管理的输入：文件缺失时，codex-helper 只使用内存中的内置 adapters，不会创建文件；文件不可读或根文档格式错误时会产生 catalog diagnostic，并保留 last-known-good catalog；没有旧 catalog 时回退到内存内置项。单个 provider 无效时只隔离并报告该 provider，其余有效 provider 继续工作。Codex-helper 绝不会替换或重写原文件。如果没有显式 adapter 匹配某个 upstream，codex-helper 会尝试常见 relay 探测：
...
`endpoint`、`headers` 和 `variables` 保留对 v0.20.3 自定义配置的只读兼容。模板支持 `baseUrl` / `base_url`、`upstreamBaseUrl` / `upstream_base_url`、`unix_now`、`unix_now_ms`、`unix_days_ago:N` 以及通过校验的自定义变量引用。对于 `openai_organization_costs`，即使读取旧内置模板，codex-helper 仍由 adapter 统一生成有界的 30 天 `start_time` 和 `limit=30` 查询窗口。

凭据边界比 v0.20.3 更严格：endpoint 不允许出现 `apiKey`、`accessToken`、`token`、`env:NAME`，也不允许通过自定义变量链间接解析到这些值。带凭据的模板只允许用于 `Authorization` 或 `X-API-Key`，并会把对应 header value 标记为 sensitive，确保 Debug 输出脱敏。其他自定义 header 和变量必须是非敏感内容。`env:NAME` 会在构建 credential generation 时登记并捕获，请求处理期间不会直接读取进程环境。危险 provider 会产生单 provider diagnostic 并被隔离，其他有效 provider 仍可刷新。
...
`new_api_user_id_env` 填写环境变量名，其值会通过固定的 `New-Api-User` header 发送。该字段只允许用于 `new_api_user_self`；一旦配置，对应环境变量就必须存在且非空。相比 legacy 自定义身份 header，应优先使用这个 kind-scoped、显式 generation-captured 的 typed field。
...
- 同一页面会显示本机增强的 CWD/transcript 元数据，以及 effective profile、model、reasoning effort、service tier、缓存读入/新建 token 与缓存命中率。对空闲会话，`b`、`M`、`E`、`f` 分别显式管理 profile/model/effort/service tier，`l`/`m`/`h`/`X` 可直接选择 reasoning effort，`R` 清除手动控制。每次 mutation 都使用 opaque runtime session identity 与 binding-revision CAS，只影响下一次请求，不会改写历史请求。`v` 可筛选存在手动控制的会话。
- `Settings` 汇总脱敏的 5m/1h 运行活动、top provider/endpoint、provider 余额、价格目录来源与最近请求缓存命中率；Integrated 模式还显示本机 config/home/request-log/database 路径。使用方向键或 `j`/`k`、PageUp/PageDown、Home 滚动。`p` 通过 lossless 配置 mutation 持久化 configured default profile，`P` 只修改 runtime default override，`R` 重载 runtime；有效 runtime override 会跨无关重载保留，profile 被删除时自动清除。Relay 诊断 `C` 与需要二次确认且可能计费的 live smoke `X`/`Y` 只在当前连接具备相应 capability 时显示。RemoteObserver 永远不显示本机路径或 mutation 快捷键。
...
`routing show` 读取持久化配置。`routing list` 和 `routing explain` 在本地编译该配置，不会查询正在运行的 daemon。
`routing explain --model <MODEL> --json` 为兼容旧脚本保留 v1 runtime-shaped JSON 字段，但会明确写入 `source = "config_only"` 和 `runtime_state_queried = false`。其中 `selected_route` 是配置层首个可用候选；实时 cooldown、capacity、balance 与 session affinity 请在 Routing TUI 或经过认证的 `GET /__codex_helper/api/v1/routing/explain` 端点查看。
...
`routing show` 回答“配置里保存了什么”。`routing explain` 回答“这份配置会先尝试什么”，包括 candidate order、route paths、conditional routing，以及 unsupported model 等配置层可见的 skip reasons；它不会伪造 daemon 的实时健康状态。
...
默认的 `requests.jsonl` 只保存提交后的结构化请求事实。需要排查上游协议兼容、请求体改写或错误响应时，可以在**启动 helper 进程前**设置：
...
如果一个本应优先 monthly providers 的 route fallback 到 paygo，先检查本地配置预览：

```bash
codex-helper routing explain --model <MODEL> --json
```

CLI 结果会明确标记为 config-only。要检查实时 cooldown、capacity、balance 和 session affinity，请使用 Routing TUI，或经过认证的 `GET /__codex_helper/api/v1/routing/explain` 端点。实时响应中优先检查这些字段：

- `selected_route.provider_endpoint_key` 和 `selected_route.preference_group` 显示运行时现在会尝试什么。Group `0` 是最高优先级组。
- `candidates[].skip_reasons` 解释 preferred candidate 为什么被跳过，例如 `unsupported_model`、`cooldown`、`usage_exhausted` 或 `runtime_disabled`。
...
在 integrated TUI 或经过认证的 loopback attached TUI 的 `2 Routing` 页面，选中候选后按 `Enter` 打开运行时操作。设置新会话偏好只影响尚无 affinity 的 session；`a`、`Backspace` 或 `Delete` 恢复容量感知的自动选择。进入 Routing 时会请求一次非强制余额/额度刷新；停留期间样本缺失、过期或约六分钟未更新时，会以至少两分钟间隔再次委托 daemon 刷新，状态栏会显示最新样本年龄。`g` 绕过 UI 自动刷新节流并强制全量刷新。Endpoint draining/disabled 是独立的维护动作，远程 observer 保持只读。这些操作都不会改写 `config.toml`，也不会静默迁移已有会话。

远程 operator clients、`RemoteObserver` 与远程 control plane 只提供查询。本机 signed operator interface 可以刷新余额，并应用临时 routing、空闲 session-affinity、session binding、runtime reload 与 default-profile 控制，但不会编辑持久 provider/routing intent；这类持久意图仍应通过本地 CLI 命令或 `config.toml` 编辑。daemon 同机的 `LocalAttached` TUI 还可以通过 `n` / `o` 与 preset 快捷键修改同一台机器的 Codex 客户端文件；这是本机 journal/CAS 文件操作，不是远程 control-plane mutation。终端客户端切换入口包括显式 `switch on/off`、integrated 或 LocalAttached TUI Settings 中的 `n` / `o`，以及使用同一 journal/CAS 契约的本地 `ch` / `ch relay` 兼容流程。`RemoteObserver` 永远不会暴露这些操作。
...
| `[codex.compaction]` | 作为 Codex 专用的 version 6 runtime 策略保留。`remote_v2_downgrade` 默认 `true`；设为 `false` 才会原样透传 v2 响应。v5 迁移会保留显式 `false`，当前 v6 加载不会重写 | 无需人工迁移；只有选中的中转原生支持 v2 时才需要设置 `[codex.compaction].remote_v2_downgrade = false` |
| `[claude.compaction]` | 这是不适用于 Claude 的契约，会被拒绝；迁移会先备份源文件并删除这张退休表 | 删除该表；压缩策略只应放在 `[codex.compaction]` |
...
| `relay_targets.*.client_preset` / `responses_websocket` | 启动先备份文件，再把两个扁平字段迁入 `[relay_targets.*.client_patch]` | 保留嵌套的部分 patch；省略字段继承 `[codex.client_patch]` |
...
| `usage_providers.json` endpoint 模板、`headers` 或 `variables` | 安全的 v0.20.3 base/time/自定义变量模板及非敏感 header 可继续读取；精确的 legacy `new_api_user_self` header `New-Api-User = "{{env:NAME}}"` 会只在内存中规范化为 `new_api_user_id_env`；URL 中凭据仍被拒绝，其它凭据只允许进入受控认证字段 | 安全文件无需重写，codex-helper 也绝不会改写这份 operator-owned JSON；编辑文件时把 URL 凭据移入受控认证字段，并优先使用 `new_api_user_id_env` 等 typed fields |
...
从 pre-v6 配置迁移且缺少 `scheduling_preset` 时会写入 `throughput-first`，保留 0.20.3 的立即 failover 行为；已有显式 preset 不变。新建及当前 version 6 route graph 缺少该字段时默认使用 `balanced`，会为本地并发容量最多等待 2 秒。
