...
macOS receipt 绑定当前登录用户的 `gui/<uid>` LaunchAgent domain 和同一用户的 Keychain；Linux receipt 绑定 `systemctl --user` 与该用户的 session bus。使用一个用户安装、再由另一个用户启动或查看时，会按设计返回身份/readiness 不匹配，而不会回退到另一份凭据存储。
