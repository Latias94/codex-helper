...
正常启动会把历史 helper 配置自动升级到当前 `version = 5` TOML 契约：

- v1-v4 或无版本的 `config.toml` 会先经过迁移与完整校验，再备份为 `config.toml.bak`，最后原子替换为 canonical version 5 TOML。
...
- 输入格式损坏、版本高于 5、迁移结果无效、源文件是符号链接，或准备迁移时源文件发生变化，都会 fail closed；源文件不会被替换。
...
启动会通过同一路径清理当前 version 5 文件中的已退休字段；`config migrate` 可以提前预览或显式执行这次清理。对已经干净的 version 5 文件显式执行迁移是 no-op，也不会覆盖已有 legacy backup。备份是安全副本，不是版本化归档；如果需要长期保留历史，请在下一次迁移或强制初始化前把备份另行保存。
...
这是 version 5 内的向后兼容字段扩展，不需要改写旧配置；但行为比旧版本更严格。升级前已经配置 helper 凭据的 provider 无需迁移。过去依赖远程匿名的 provider 必须补凭据，或显式记录匿名意图：
...
- 优先使用 `*_env` 字段，避免 secret 写入 `~/.codex-helper/config.toml`。
  config 里的值是凭据引用名，不是密钥本身。解析顺序是 inline 值、运行 codex-helper 的进程环境、显式引用的客户端凭据字段。Codex 会在 `$CODEX_HOME/auth.json` 顶层查找完全同名的字符串字段；Claude 会在 `$CLAUDE_HOME/settings.json` 的 `env` 对象中查找。未配置 `*_env` 时不会扫描这些文件或猜测字段。
...
Windows 上，新版 `service install` 不再注册 LocalSystem SCM service，而是为当前登录用户注册 SID-scoped 的 per-user Scheduled Task；实际任务名形如 `codex-helper-S-1-5-21-...`，`service start/stop/status/uninstall` 会自动解析当前 Principal SID，日常 CLI 使用不需要知道该名字。任务使用 `InteractiveToken` 和 `LeastPrivilege`，在该用户登录时触发，也可通过 `service start/restart` 按需运行。Task definition 只记录 executable、helper home、client home、监听地址和端口，不记录凭据值。升级旧 SCM 安装时，请用新版 binary 在一次提升权限的 PowerShell 中执行：
...
安装会先完成 executable、路径、SID、PowerShell/ScheduledTasks 命令和 XML 写回预检，再注册并回读校验新的 SID-scoped task；只有 owner SID、启动 action、登录 trigger 和最小权限均匹配后，才会停用当前 SID 所属的旧固定名 task 并停止/删除 legacy SCM service。任何权限、查询、注册或校验错误都会失败关闭；若迁移在旧运行体停用期间失败，会恢复旧 task/重启旧 SCM，只有无法证明旧运行体已恢复时才保留已验证的新 task 作为可运行 fallback。固定名 `codex-helper` task 若属于其他 SID，绝不会被覆盖或删除。成功后 `service status` 的 detail 应显示 `SID-scoped per-user scheduled task`；如果仍显示 `legacy LocalSystem SCM service` 或 `legacy fixed-name per-user scheduled task`，请在提升权限终端重跑安装。Task definition 不会捕获当前 PowerShell 窗口的临时 `$env:NAME`；请使用持久的用户环境、上述精确同名客户端凭据文件 fallback，或受本机权限保护的 inline 配置。显式引用的凭据无法解析或不是合法 header 值时，HTTP、WebSocket、probe、live smoke 和余额刷新都会在访问上游前失败关闭。
...
正常启动，包括默认打开 TUI 的路径，都使用 canonical `~/.codex-helper/config.toml` 的 `version = 5` 契约。启动发现旧版本/无版本 TOML，或者 TOML 不存在但有 `config.json` 时，会执行[配置自动迁移](#配置自动迁移)中说明的校验与迁移。没有 helper 配置时，`config init` 会创建当前模板；只有历史 JSON 时，它会迁移该文件并输出报告。`--force` 只会在写入 `config.toml.bak` 后替换已有 canonical 文件。
...
`~/.codex-helper/config.toml` 中的 `version = 5` 仍是唯一公开 runtime 契约。更早的有版本或无版本 TOML，以及历史 `config.json`，是一次性迁移输入，而不是长期兼容格式：启动会转换它们，创建与源文件对应的 `.bak`，之后只加载 canonical version 5 TOML。Runtime 不会并行维护一套 legacy reader。

迁移会尽量保留当前契约可表达的 provider/routing intent，移除已知退休配置，对已知有损转换以及原样保留的未知 root/service 字段发出 warning，并在替换前校验 version 5 结果。只有当 `[retry.upstream]` 不存在时，`[retry]` 下的扁平 `max_attempts`、`backoff_ms`、`backoff_max_ms`、`jitter_ms`、`on_status`、`on_class` 和 `strategy` 才会迁入该表；如果 `[retry.upstream]` 已存在，它会作为完整历史 override 保留，所有 flat retry 字段都会被忽略并产生 warning。高度定制的文件建议先运行 `config migrate --dry-run` 审阅转换；未来版本 schema 永远不会被自动降级。

普通 0.20.3 version 5 文件中的 provider、endpoint、route graph、retry profile、notification、fleet 和 service-status 设置仍可继续使用。但同一个 version 5 契约中曾发布过若干现已退休的可选字段；单看版本号无法识别它们。启动会先创建 `config.toml.bak`，再精确删除所有命中的 helper-config 路径后加载；显式 typed save 仍拒绝直接覆盖未清理的源文件，避免无关命令静默擦除字段。
...
| 远程 `relay_targets.*` 缺少 `admin_url`，或使用非 loopback HTTP admin URL / 缺少 token env | 书签仍属于 version 5，但解析 target 时失败关闭；远程 admin authority 永远不会从 `proxy_url` 或响应中推导 | 显式配置可信 HTTPS `admin_url` 和有效 `admin_token_env`，或把可信隧道终止到 loopback；不会自动改写 |
...
| 同一显式 `limit_group` 混有缺失或不同的 `max_concurrent_requests` | Route graph 编译及正常 version 5 加载失败关闭，源文件不会被改写 | 让组内每个候选使用相同上限，或把独立的 20 槽与 15 槽账号拆到不同 group |
