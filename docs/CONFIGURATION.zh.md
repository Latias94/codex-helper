...
官方 compact 命中通常会在 codex-helper 日志中显示为 `POST /responses/compact`。普通本地 fallback 压缩只会显示为普通 `POST /responses` 请求。Remote compaction v2 如果由 Codex 启用，也会走普通 `/responses`，通过结构化 `compaction_trigger` input item 表达，而不是 `/responses/compact`；helper 会在日志里标记 `codex_bridge.remote_compaction_v2_request = true`，并按 state-bound route-continuity 规则处理。当前 helper 预设不会启用 v2。启用 `responses_websocket` 后，普通 turn streaming 会走 WebSocket `GET /responses` 风格 upgrade，而不是 HTTP `POST /responses`。
...
对 Codex remote compaction，helper 会把带有 `encrypted_content`、`previous_response_id` 或 `compaction_summary` 这类字段的 v1 compact，以及带结构化 `compaction_trigger` 的 v2 compact，视为 provider-state-bound。这类请求必须使用已知 route affinity。如果 helper 重启或配置变化后没有可证明的 affinity，helper 会返回明确的连续性错误，而不是静默选择另一个 provider endpoint。如果已知 affinity endpoint 自身失败，provider-state-bound compact 仍会 fail closed；不带这类状态字段的 v1 compact 仍可按 route policy 走普通 provider fallback。
