...
Resident runtime 会在 `~/.codex-helper/run/` 下尽力写入 owner marker，让 `daemon status` 能区分 manual CLI、supervisor 以及未来桌面/托盘拥有的 sidecar。这些 marker 只是可观测性元数据：即使缺失、损坏或清理失败，也不应该阻止代理启动、退出或被显式停止。desktop-managed sidecar 模式在有可见桌面/托盘外壳前保持隐藏；普通 `serve` 启动默认仍然不是常驻模式。
...
- TUI Requests 页会在列表的 `RG` 列显示命中标记；详情里的 `Retry / route chain` 会显示 `decision=failed_reasoning_guard`、`class=reasoning_guard_triggered` 和 `reason=reasoning_tokens=<命中值>`。预算耗尽后放行的最后一次响应会按正常完成记录，同时 retry trace 会有 `action=exhausted-pass`。
...
请求触发的余额刷新默认会先延迟 60 秒合并，同一 provider 默认至少间隔 600 秒才会再次自动查询；显式配置的 `poll_interval_secs` 小于 120 秒时会被抬到 120 秒。手动刷新仍可主动查询，但会尊重已确认的终态错误和当前周期已耗尽保护。
...
常驻 proxy runtime 只拥有一个 quota sampler。它会在启动时刷新一次，之后默认约每 5 分钟调度一轮，并加入最多 10% 的正向 jitter；provider polling throttle、reset/耗尽抑制和 `Retry-After` 仍可能推迟真正的 HTTP 请求。所有 provider 连续失败时使用有上限的指数退避。有效的语义观测会写入有界 checkpoint `~/.codex-helper/state/quota-samples.json` 并跨重启续接；失败和离线空档不会被插值。Attached TUI 只通过目标 daemon 读取和刷新，因此不会启动第二个采样器。
...
- 在 Usage 页按 `g` 会要求 daemon 强制刷新余额；attached 模式会把请求发给目标 daemon。单个 provider 失败时会保留 cached value，并明确显示 offline/stale，不会清空其他 pool 或打断 redraw。
- 远端 pool counter 是其 account/key/subscription scope 下总消耗的事实源，可能包含其他电脑的请求。本地 request ledger 只负责本 daemon 的 project 归因。对账使用 `external = max(remote - local, 0)`；本地高于远端时保留负 signed gap；绝不会用远端差额放大本地请求价格，也不会把 external 用量摊到本地项目。
- Project 行会尽量把新请求归一到 Git root，并保留 fallback/unknown 和 omitted 行。新请求成本会保留 selected tier 和 effective pricing source/generation；旧记录如果只能重建估算，就会降低 coverage，而不会伪装成捕获到的账单事实。远端 quota 面板下面仍保留本地日 provider/station/model/session 上下文和 24 小时活跃度。
...
- 对账要求远端/本地 window 对齐、单位为 USD、conversion generation 相同，并且 request-log 与价格 coverage 足够。Raw unit、divisor 变化、不兼容 generation、window mismatch、截断/重建记录、未定价或未匹配请求、dedupe/boundary 不确定性和算术 overflow 都会保留已有数值，但把差额标成 unavailable 或 incomplete。Coverage warning 不代表更早用量为零。
...
- `Routing` 页面保留紧凑余额上下文和 route eligibility 控制。判断 pool 消耗与 pace 看 TUI `Usage`；判断路由可用性看 Routing/Stations。

旧 `[ui.usage_forecast]` 配置块仍可在读取既有配置时兼容接受，但它不再定义另一套权威 pacing 模型。新的 pace 由 daemon 中带类型的 quota pool window、reset、samples 和 conversion generation 决定。

不依赖本地 request log，也可以直接读取同一份 daemon-owned DTO：
...
`--target` 会解析已配置的本地或远端 relay admin endpoint。JSON 模式返回 daemon 的有界 quota analytics，CLI 不会自行重算 slope、reset boundary 或 project reconciliation。
...
- 常驻 daemon 启动时会检查 BaseLLM，之后约每 6 小时使用 conditional request 再检查。Candidate 只有通过有界解析、语义和经济变化校验后才会成为 last-known-good（LKG）；失败保留旧 LKG，可疑经济变化进入 quarantine，等待显式批准。自动刷新永远不会写 `pricing_overrides.toml`。
- Operator commands：
...
- Desktop provider / routing forms：暴露常见编辑，同时把高级 route graph 字段保留给 CLI 或 raw TOML。
...
- 从 TUI 或桌面表单静默写出 legacy station schema；
