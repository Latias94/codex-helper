...
Switch 永远不会修改 `~/.codex/auth.json`、`models_cache.json`、Codex SQLite、无关 providers、feature flags、compaction 设置、WebSocket 设置或 hosted-tool 设置。Provider capability 来自选中 provider 的契约和实时观测，而不是 switch 配置。
