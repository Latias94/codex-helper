...
0.20.3 的 `~/.codex-helper/state/session-route-affinities.json` 是旧 helper 生成的运行时状态，不是用户配置；新版不会把它导入 `state.sqlite`，也不会改写或删除它。升级后，旧进程中尚未结束的多 endpoint 会话没有可恢复的 affinity。继续这类会话时，先用相同 session key 完成一次普通 Responses 请求，让新版记录新的 provider endpoint，再发送依赖上游状态的 compact 请求；`hard` affinity 在重新建立记录前会明确拒绝缺少 affinity 的 state-bound compact。
