...
在 balance adapter templates 中，`{{base_url}}` 会被规范化为不带结尾 `/v1`。只有当 balance endpoint 确实位于和模型请求相同的 `/v1` 前缀下时，才使用 `{{upstream_base_url}}`。官方 usage/cost APIs 需要查询窗口时，可以使用 `{{unix_now}}`、`{{unix_now_ms}}` 和 `{{unix_days_ago:30}}` 这类时间 helpers。自定义 `headers` 可以通过 `{{env:NAME}}` 引用环境变量；credential 仍由环境管理，绝不会写入 RuntimeStore。
...
| `headers` | 可选 adapter headers；值可用 `{{env:NAME}}`，不会持久化 credential |
