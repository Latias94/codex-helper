...
该检查只加载、编译配置并解析受支持的凭据输入，不打开 runtime store/listener，也不发送任何上游请求。`ready` 和 `degraded` 以状态码 0 退出；`blocked` 输出脱敏报告并以状态码 1 退出。经过验证的只读 mount、ACL 和替换流程见 [Docker Compose 部署](DOCKER_COMPOSE.md)。
