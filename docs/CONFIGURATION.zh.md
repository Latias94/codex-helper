...
0.20.3 及更早版本使用另一套 switch 实现，并把恢复数据保存在 `~/.codex/codex-helper-switch-state.json`。该文件可能包含原 provider selector；使用过旧 bridge preset 时，还可能包含原始 `auth.json` 内容。当前 switch journal 不会读取这个文件，也刻意不读取 Codex auth 状态。

如果 legacy state 文件存在，请按下面的顺序升级：

1. 替换旧 binary 前，先用创建该 legacy state 的版本运行 `codex-helper switch off`。它会恢复旧 selector/provider stanza，以及旧 switch 管理过的 auth facade。
2. 在旧 `switch off` 成功前保留 `~/.codex/codex-helper-switch-state.json`。不要删除、编辑、分享或粘贴到 issue 中，因为它可能包含认证材料。
3. 升级 codex-helper，先运行 `codex-helper switch status`，确认后再按需运行新版 `switch on`。

如果已经安装新版，请先临时运行上一版本 binary 的 `switch off`，再使用新版 switch；不要并发运行新旧 switch 命令。新版只检测 legacy state 路径是否存在，不读取其内容；它会报告 `recovery_required`，也不会认领或覆盖旧 state/provider stanza。
...
对于带 session id 的每个请求，codex-helper 使用 `session_id + service + route_graph_key` 作为 affinity key。Route key 是带版本的 canonical SHA-256 digest；调度 preset、provider 展示 alias 和 route node 展示 metadata 不会改变它，路由选择规则、provider endpoint identity 或配置内 `auth_token` / `api_key` 凭据则会改变它。客户端透传的账号 headers 和外部 credentials fallback 不属于该持久 identity，因此切换其中任一账号时必须开启新会话。
...
- 如果 provider tags、route node strategy、children、entry、provider endpoint identity 或配置内 `auth_token` / `api_key` 凭据改变，route graph key 会改变，旧 affinity 不再匹配；仅修改 `scheduling_preset` 或展示 alias/metadata 时会保留；
...
- 远端 quota observation 由 daemon-owned 后台 sampler 刷新；attached TUI 和桌面端保持只读，不会发起远端刷新 mutation。单个 provider 失败时会保留最后已提交的值，并明确显示 offline/stale，不会清空其他 pool 或打断 redraw。
...
- `Routing` 页面保留紧凑余额上下文和 route eligibility 控制。判断 pool 消耗与 pace 看 TUI `Usage`；判断 provider endpoint 路由可用性看 Routing。
...
这些 operator clients 与远程 control plane 只提供查询。持久 provider/routing intent 应通过本地 CLI 命令或 `config.toml` 编辑。attached TUI 不处理 `n` / `o`，也不会检查或修改本机 Codex 配置。终端场景的客户端切换只能通过独立且显式的本地 `switch on/off` CLI，或 integrated local TUI 的 Settings 页面中的 `n` / `o`；两者都不是远程 control-plane 操作。
...
普通 0.20.3 version 5 文件中的 provider、endpoint、route graph、retry profile、notification、fleet 和 service-status 设置仍可继续使用。但同一个 version 5 契约中曾发布过若干现已退休的可选字段；单看版本号无法识别它们。启动与 typed config 保存会拒绝这些精确路径并一次列出所有命中项，不会再静默丢弃；源文件保持不变。

| 已退休输入 | 当前行为 | 升级操作 |
| --- | --- | --- |
| `[codex.client_patch]` | 启动时拒绝；helper config 不再管理 preset、auth facade、compaction、hosted-tool switch 或 WebSocket patch | 删除整张表，只使用上文显式 URL switch |
| `[codex.compaction]` / `[claude.compaction]` | 启动时拒绝任一表；v0.20.3 的共享 schema 曾接受 Claude 表，但它从未产生 Claude 运行时效果 | 删除整张表；helper 不再执行 remote-v2-to-v1 downgrade |
| `[ui.usage_forecast]` | 启动时拒绝；旧本地 forecast 已移除 | 删除整张表，改用已提交的 quota pace 和 reset-window 视图 |
| `codex.profiles.*.station` / `claude.profiles.*.station` | 启动时拒绝每个命中的 profile 字段 | 删除该字段，通过 service route graph 表达 provider selection |
| `[retry].allow_cross_station_before_first_output` | 启动时拒绝该退休 retry 字段 | 删除该字段；failover 由 canonical route/retry policy 控制 |
| `relay_targets.*.client_preset` / `responses_websocket` | 启动时拒绝每个命中的 relay-target 字段 | 删除这些字段；relay bookmark 只保存 network/admin 连接信息 |
