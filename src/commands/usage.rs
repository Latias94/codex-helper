...
pub async fn handle_usage_cmd(cmd: UsageCommand) -> CliResult<()> {
    ...
    handle_usage_cmd_with_writer(cmd, &mut stdout).await
}

async fn handle_usage_cmd_with_writer(cmd: UsageCommand, writer: &mut dyn Write) -> CliResult<()> {
    let cmd = match extract_quota_command(cmd) {
        Ok((target, json)) => return handle_usage_quota(target, json, writer).await,
        Err(cmd) => *cmd,
    };
...
fn extract_quota_command(cmd: UsageCommand) -> Result<(String, bool), Box<UsageCommand>> {
    match cmd {
        ...
        request_log_command => Err(Box::new(request_log_command)),
    }
}
...
async fn handle_usage_quota(
    ...
    let snapshot = client
        .snapshot(1, 1)
        .await
        .map_err(|_| quota_error(&resolved_name, "snapshot_fetch_failed"))?;
    let quota = &snapshot.snapshot.quota_analytics;
    ...
    if json {
        ...
        write!(
            writer,
            "{}",
            quota_text(&resolved_name, &snapshot.service_name, quota)
        )
        .map_err(|_| quota_error(&resolved_name, "output_write_failed"))?;
    }
    Ok(())
...
mod tests {
    ...
    use codex_helper_core::config::{
        ProxyConfigV4, RelayTargetConfig, ServiceKind, save_config_v4,
    };
    use codex_helper_core::dashboard_core::snapshot::DashboardSnapshot;
    use codex_helper_core::dashboard_core::{ApiV1Snapshot, WindowStats};
    ...
    fn control_plane_snapshot(quota_analytics: QuotaAnalyticsView) -> ApiV1Snapshot {
        ApiV1Snapshot {
            api_version: 1,
            ...
            snapshot: DashboardSnapshot {
                ...
                session_model_overrides: Default::default(),
                session_station_overrides: Default::default(),
                session_route_target_overrides: Default::default(),
                session_effort_overrides: Default::default(),
                session_service_tier_overrides: Default::default(),
                session_stats: Default::default(),
                station_health: Default::default(),
                provider_balances: Default::default(),
                provider_balance_history: Default::default(),
                health_checks: Default::default(),
                lb_view: Default::default(),
    ...
    #[test]
    fn quota_command_is_extracted_before_request_log_commands() {
        assert_eq!(
    ...
    async fn quota_json_runs_the_command_path_against_a_mock_control_plane() {
        ...
        let mut wire_body =
            serde_json::to_value(control_plane_snapshot(expected.clone())).expect("snapshot JSON");
        let wire_object = wire_body.as_object_mut().expect("snapshot object");
        wire_object.remove("runtime_loaded_at_ms");
        wire_object.remove("runtime_source_mtime_ms");
        wire_object.remove("default_profile");
        wire_object.insert(
            "validator".to_string(),
        ...
        let wire_pool = wire_body["snapshot"]["quota_analytics"]["pools"][0]
            .as_object_mut()
        ...
        let mut config = ProxyConfigV4::default();
        ...
        let result = handle_usage_cmd_with_writer(cmd, &mut stdout).await;
        server.abort();
        let _ = server.await;
        ...
        assert_eq!(
            observed.0,
            "/__codex_helper/api/v1/snapshot?recent_limit=1&stats_days=1"
        );
